...
module tb_comparator_multi_branch_alloc_status_reuse;
...
localparam [`REQ_ID_W-1:0] REUSE2_REQ_ID = 4'h7;
localparam [`REQ_ID_W-1:0] POST_FULL_REQ_ID = 4'h8;
...
initial begin
    ...
    issue_req_expect_grant("post_flush_reuse_alloc_second", REUSE2_REQ_ID, REUSE2_BRANCH_ID, REUSE2_NODE_ID, 3'd5);
    if ((reuse2_sram_id_r != victim1_sram_id_r) ||
        (reuse2_bank_id_r != victim1_bank_id_r) ||
        (reuse2_subbank_start_r != victim1_subbank_start_r) ||
        (reuse2_group_len_r != victim1_group_len_r)) begin
        $fatal(1, "second post-flush allocation did not reuse the second reclaimed victim range");
    end

    issue_req_expect_deny("post_flush_full_again", POST_FULL_REQ_ID, REUSE2_BRANCH_ID, 4'd2);

    query_bank_bitmap(victim0_sram_id_r, victim0_bank_id_r);
    expect_range_occupied("victim0_after_reuse", query_bitmap_r, victim0_subbank_start_r, victim0_group_len_r);
    query_bank_bitmap(victim1_sram_id_r, victim1_bank_id_r);
    expect_range_occupied("victim1_after_reuse", query_bitmap_r, victim1_subbank_start_r, victim1_group_len_r);
    query_bank_bitmap(survivor_sram_id_r, survivor_bank_id_r);
    expect_range_occupied("survivor_after_reuse", query_bitmap_r, survivor_subbank_start_r, survivor_group_len_r);
    ...
    expect_free_list_range_state("victim1_free_list_after_reuse", victim1_sram_id_r, victim1_bank_id_r,
                                 victim1_subbank_start_r, victim1_group_len_r, 1'b0,
                                 REUSE2_REQ_ID, REUSE2_BRANCH_ID, REUSE2_NODE_ID);
    expect_free_list_range_state("survivor_free_list_after_reuse", survivor_sram_id_r, survivor_bank_id_r,
                                 survivor_subbank_start_r, survivor_group_len_r, 1'b0,
                                 TEST_REQ_ID, SURVIVOR_BRANCH_ID, SURVIVOR_NODE_ID);

    $display("tb_comparator_multi_branch_alloc_status_reuse PASS");
    $finish;
end
