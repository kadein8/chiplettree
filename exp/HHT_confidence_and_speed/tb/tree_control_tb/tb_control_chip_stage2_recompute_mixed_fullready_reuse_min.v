...
module tb_control_chip_stage2_recompute_mixed_fullready_reuse_min;
...
localparam [`NODE_ID_W-1:0] A1_PARENT_NODE_ID = 4'd5;
localparam [`TOKEN_ID_W-1:0] A1_TOKEN_ID = 16'ha101;
localparam [`TOKEN_ID_W-1:0] A_REF_TOKEN_ID = 16'h9a10;
localparam [`POSITION_ID_W-1:0] A_REF_POS = 12'h020;
localparam [CONF_W-1:0] A1_CONF = 8'd208;

localparam [`NODE_ID_W-1:0] A2_PARENT_NODE_ID = 4'd6;
localparam [`TOKEN_ID_W-1:0] A2_TOKEN_ID = 16'ha202;
localparam [CONF_W-1:0] A2_CONF = 8'd206;

localparam [`NODE_ID_W-1:0] B1_PARENT_NODE_ID = 4'd7;
localparam [`TOKEN_ID_W-1:0] B1_TOKEN_ID = 16'hb101;
localparam [`TOKEN_ID_W-1:0] B_REF_TOKEN_ID = 16'h9b10;
localparam [`POSITION_ID_W-1:0] B_REF_POS = 12'h021;
localparam [CONF_W-1:0] B1_CONF = 8'd204;

localparam [`NODE_ID_W-1:0] B2_PARENT_NODE_ID = 4'd8;
localparam [`TOKEN_ID_W-1:0] B2_TOKEN_ID = 16'hb202;
localparam [CONF_W-1:0] B2_CONF = 8'd202;

localparam [`SRAM_ADDR_W-1:0] EXP_SRC_ADDR = {2'd0, 4'd2, 5'd6, 8'h00, 4'h0};
localparam [`REQ_ID_W-1:0] EXP_REQ_ID = 4'h6;
localparam [`SRAM_WDATA_W-1:0] A_FULL_WDATA =
    128'h1111_aaaa_2222_bbbb_3333_cccc_4444_dddd;
localparam [`SRAM_WDATA_W-1:0] B_PARTIAL_WDATA =
    128'h5555_1111_6666_2222_7777_3333_8888_4444;
localparam [`SRAM_WDATA_W-1:0] B_FULL_WDATA =
...
reg pred_b1_seen_r;
reg pred_b2_seen_r;
integer recompute_req_count_a1_r;
integer recompute_req_count_a2_r;
integer recompute_req_count_b1_r;
integer recompute_req_count_b2_r;
reg stale_seen_r;
reg a_full_write_seen_r;
reg b_partial_write_seen_r;
reg b_full_write_seen_r;
reg issue_a1_seen_r;
reg issue_a2_seen_r;
reg issue_b1_seen_r;
reg issue_b2_seen_r;
...
reg wb_b1_seen_r;
reg wb_b2_seen_r;
reg kv_a_full_seen_r;
reg kv_b_partial_seen_r;
reg kv_b_full_seen_r;
integer transform_read_count_r;
integer recompute_done_count_r;
wire a_full_write_fire_w;
wire b_partial_write_fire_w;
wire b_full_write_fire_w;
...
assign a_full_write_fire_w =
    mem_req_valid[0] && mem_req_ready[0] &&
    mem_req_write[0] &&
    (mem_req_addr[0 +: `SRAM_ADDR_W] == EXP_SRC_ADDR) &&
    (mem_req_wdata[0 +: `SRAM_WDATA_W] == A_FULL_WDATA);
assign b_partial_write_fire_w =
    mem_req_valid[0] && mem_req_ready[0] &&
    mem_req_write[0] &&
    (mem_req_addr[0 +: `SRAM_ADDR_W] == EXP_SRC_ADDR) &&
    (mem_req_wdata[0 +: `SRAM_WDATA_W] == B_PARTIAL_WDATA);
assign b_full_write_fire_w =
    mem_req_valid[0] && mem_req_ready[0] &&
    mem_req_write[0] &&
    (mem_req_addr[0 +: `SRAM_ADDR_W] == EXP_SRC_ADDR) &&
    (mem_req_wdata[0 +: `SRAM_WDATA_W] == B_FULL_WDATA);
...
task drive_candidate_window_once;
    ...
    begin
        ...
        draft_parent_node_id[(1*`NODE_ID_W) +: `NODE_ID_W] = selected_parent_node_id;
        draft_token_id[(1*`TOKEN_ID_W) +: `TOKEN_ID_W] = selected_token_id;
        draft_referenced_token_id[(1*`TOKEN_ID_W) +: `TOKEN_ID_W] = selected_referenced_token_id;
        draft_referenced_position[(1*`POSITION_ID_W) +: `POSITION_ID_W] = selected_referenced_position;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        pred_b1_seen_r <= 1'b0;
        pred_b2_seen_r <= 1'b0;
        recompute_req_count_a1_r <= 0;
        recompute_req_count_a2_r <= 0;
        recompute_req_count_b1_r <= 0;
        recompute_req_count_b2_r <= 0;
        stale_seen_r <= 1'b0;
        a_full_write_seen_r <= 1'b0;
        b_partial_write_seen_r <= 1'b0;
        b_full_write_seen_r <= 1'b0;
        issue_a1_seen_r <= 1'b0;
        issue_a2_seen_r <= 1'b0;
        issue_b1_seen_r <= 1'b0;
        issue_b2_seen_r <= 1'b0;
        wb_a1_seen_r <= 1'b0;
        wb_a2_seen_r <= 1'b0;
        wb_b1_seen_r <= 1'b0;
        wb_b2_seen_r <= 1'b0;
        kv_a_full_seen_r <= 1'b0;
        kv_b_partial_seen_r <= 1'b0;
        kv_b_full_seen_r <= 1'b0;
        transform_read_count_r <= 0;
        recompute_done_count_r <= 0;
    end else begin
        if (pred_valid && pred_ready &&
            (pred_source_id == SEL_SOURCE_ID) &&
            pred_is_last_in_window) begin
            if ((pred_parent_node_id == A1_PARENT_NODE_ID) &&
                (pred_token_id == A1_TOKEN_ID) &&
                (pred_referenced_token_id == A_REF_TOKEN_ID) &&
                (pred_referenced_position == A_REF_POS) &&
                (pred_confidence == A1_CONF)) begin
                pred_a1_seen_r <= 1'b1;
            end
            if ((pred_parent_node_id == A2_PARENT_NODE_ID) &&
                (pred_token_id == A2_TOKEN_ID) &&
                (pred_referenced_token_id == A_REF_TOKEN_ID) &&
                (pred_referenced_position == A_REF_POS) &&
                (pred_confidence == A2_CONF)) begin
                pred_a2_seen_r <= 1'b1;
            end
            if ((pred_parent_node_id == B1_PARENT_NODE_ID) &&
                (pred_token_id == B1_TOKEN_ID) &&
                (pred_referenced_token_id == B_REF_TOKEN_ID) &&
                (pred_referenced_position == B_REF_POS) &&
                (pred_confidence == B1_CONF)) begin
                pred_b1_seen_r <= 1'b1;
            end
            if ((pred_parent_node_id == B2_PARENT_NODE_ID) &&
                (pred_token_id == B2_TOKEN_ID) &&
                (pred_referenced_token_id == B_REF_TOKEN_ID) &&
                (pred_referenced_position == B_REF_POS) &&
                (pred_confidence == B2_CONF)) begin
                pred_b2_seen_r <= 1'b1;
            end
        end
        ...
        if (recompute_req_valid && recompute_req_ready &&
            ...
            if ((recompute_req_token_id == A1_TOKEN_ID) &&
                (recompute_req_referenced_position == A_REF_POS)) begin
                recompute_req_count_a1_r <= recompute_req_count_a1_r + 1;
            end
            if ((recompute_req_token_id == A2_TOKEN_ID) &&
                (recompute_req_referenced_position == A_REF_POS)) begin
                recompute_req_count_a2_r <= recompute_req_count_a2_r + 1;
            end
            if ((recompute_req_token_id == B1_TOKEN_ID) &&
                (recompute_req_referenced_position == B_REF_POS)) begin
                recompute_req_count_b1_r <= recompute_req_count_b1_r + 1;
            end
            if ((recompute_req_token_id == B2_TOKEN_ID) &&
                (recompute_req_referenced_position == B_REF_POS)) begin
                recompute_req_count_b2_r <= recompute_req_count_b2_r + 1;
            end
        end

        if (a_full_write_fire_w) begin
            a_full_write_seen_r <= 1'b1;
        end
        if (b_partial_write_fire_w) begin
            b_partial_write_seen_r <= 1'b1;
        end
        if (b_full_write_fire_w) begin
            b_full_write_seen_r <= 1'b1;
        end

        if (issue_valid && issue_ready &&
            (issue_src_addr == EXP_SRC_ADDR) &&
            (issue_req_id == EXP_REQ_ID)) begin
            if ((issue_token_id == A1_TOKEN_ID) &&
                (issue_parent_node_id == A1_PARENT_NODE_ID) &&
                (issue_confidence == A1_CONF)) begin
                if (!(a_full_write_seen_r || a_full_write_fire_w)) begin
                    $fatal(1, "A1 issue fired before direct-full KV write");
                end
                issue_a1_seen_r <= 1'b1;
            end
            if ((issue_token_id == A2_TOKEN_ID) &&
                (issue_parent_node_id == A2_PARENT_NODE_ID) &&
                (issue_confidence == A2_CONF)) begin
                issue_a2_seen_r <= 1'b1;
            end
            if ((issue_token_id == B1_TOKEN_ID) &&
                (issue_parent_node_id == B1_PARENT_NODE_ID) &&
                (issue_confidence == B1_CONF)) begin
                if (!(b_partial_write_seen_r || b_partial_write_fire_w)) begin
                    $fatal(1, "B1 issue fired before partial KV write");
                end
                issue_b1_seen_r <= 1'b1;
            end
            if ((issue_token_id == B2_TOKEN_ID) &&
                (issue_parent_node_id == B2_PARENT_NODE_ID) &&
                (issue_confidence == B2_CONF)) begin
                issue_b2_seen_r <= 1'b1;
            end
        end
        ...
        if (mem_req_valid[0] && mem_req_ready[0] &&
            ...
            transform_read_count_r <= transform_read_count_r + 1;
        end

        if (wb_valid && (wb_status == STATUS_OK)) begin
            if ((wb_token_id == A1_TOKEN_ID) &&
                (wb_data == A_FULL_WDATA)) begin
                wb_a1_seen_r <= 1'b1;
            end
            if ((wb_token_id == A2_TOKEN_ID) &&
                (wb_data == A_FULL_WDATA)) begin
                wb_a2_seen_r <= 1'b1;
            end
            if ((wb_token_id == B1_TOKEN_ID) &&
                (wb_data == B_PARTIAL_WDATA)) begin
                wb_b1_seen_r <= 1'b1;
            end
            if ((wb_token_id == B2_TOKEN_ID) &&
                (wb_data == B_FULL_WDATA)) begin
                wb_b2_seen_r <= 1'b1;
            end
        end
        ...
        if (kv_lookup_valid && kv_lookup_ready &&
            ...
            if ((kv_lookup_token_id == A_REF_TOKEN_ID) &&
                (kv_lookup_position_id == A_REF_POS) &&
                !kv_lookup_partial_ready &&
                kv_lookup_full_ready) begin
                kv_a_full_seen_r <= 1'b1;
            end
            if ((kv_lookup_token_id == B_REF_TOKEN_ID) &&
                (kv_lookup_position_id == B_REF_POS) &&
                kv_lookup_partial_ready &&
                !kv_lookup_full_ready) begin
                kv_b_partial_seen_r <= 1'b1;
            end
            if ((kv_lookup_token_id == B_REF_TOKEN_ID) &&
                (kv_lookup_position_id == B_REF_POS) &&
                kv_lookup_partial_ready &&
                kv_lookup_full_ready) begin
                kv_b_full_seen_r <= 1'b1;
            end
        end
    end
end
...
initial begin
    ...
    if (tree_busy !== 1'b1) $fatal(1, "A1 tree control did not enter busy");
    drive_candidate_window_once(A1_PARENT_NODE_ID, A1_TOKEN_ID, A_REF_TOKEN_ID,
                                A_REF_POS, A1_CONF);
    cycle_count = 0;
    while (!pred_a1_seen_r && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!pred_a1_seen_r) $fatal(1, "A1 prediction-to-tree handshake was not observed");
    cycle_count = 0;
    while ((recompute_req_count_a1_r != 1) && (cycle_count < 12)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (recompute_req_count_a1_r != 1) $fatal(1, "A1 recompute request handshake was not observed");
    if (!stale_seen_r) $fatal(1, "stale hit was not observed");
    if (!debug_recompute_busy) $fatal(1, "A1 recompute control did not enter busy");
    cycle_count = 0;
    while (!recompute_resp_ready && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!recompute_resp_ready) $fatal(1, "A1 recompute response channel was not ready");
    send_full_kv_response_once(A_FULL_WDATA);
    cycle_count = 0;
    while (!a_full_write_seen_r && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!a_full_write_seen_r) $fatal(1, "A1 direct-full KV SRAM write was not observed");
    query_kv_tables_once(A_REF_TOKEN_ID, A_REF_POS);
    if (!kv_a_full_seen_r) $fatal(1, "A group KV table lookup did not return full-ready state");
    cycle_count = 0;
    while (!issue_a1_seen_r && (cycle_count < 12)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!issue_a1_seen_r) $fatal(1, "A1 issue metadata did not match selected token");
    cycle_count = 0;
    while (!wb_a1_seen_r && (cycle_count < 40)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!wb_a1_seen_r) $fatal(1, "A1 writeback payload was not observed");
    cycle_count = 0;
    while ((tree_busy !== 1'b0) && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (tree_busy !== 1'b0) $fatal(1, "A1 tree control did not return to idle");
    ...
    if (tree_busy !== 1'b1) $fatal(1, "A2 tree control did not enter busy");
    drive_candidate_window_once(A2_PARENT_NODE_ID, A2_TOKEN_ID, A_REF_TOKEN_ID,
                                A_REF_POS, A2_CONF);
    cycle_count = 0;
    while (!pred_a2_seen_r && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!pred_a2_seen_r) $fatal(1, "A2 prediction-to-tree handshake was not observed");
    cycle_count = 0;
    while (!issue_a2_seen_r && (cycle_count < 12)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!issue_a2_seen_r) $fatal(1, "A2 full-ready reuse issue was not observed");
    cycle_count = 0;
    while (!wb_a2_seen_r && (cycle_count < 40)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!wb_a2_seen_r) $fatal(1, "A2 writeback payload was not observed");
    cycle_count = 0;
    while ((tree_busy !== 1'b0) && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (tree_busy !== 1'b0) $fatal(1, "A2 tree control did not return to idle");
    if (recompute_req_count_a2_r != 0) $fatal(1, "A2 should not emit recompute request on full-ready reuse");
    ...
    if (tree_busy !== 1'b1) $fatal(1, "B1 tree control did not enter busy");
    drive_candidate_window_once(B1_PARENT_NODE_ID, B1_TOKEN_ID, B_REF_TOKEN_ID,
                                B_REF_POS, B1_CONF);
    cycle_count = 0;
    while (!pred_b1_seen_r && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!pred_b1_seen_r) $fatal(1, "B1 prediction-to-tree handshake was not observed");
    cycle_count = 0;
    while ((recompute_req_count_b1_r != 1) && (cycle_count < 12)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (recompute_req_count_b1_r != 1) $fatal(1, "B1 recompute request handshake was not observed");
    cycle_count = 0;
    while (!recompute_resp_ready && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!recompute_resp_ready) $fatal(1, "B1 recompute response channel was not ready for partial");
    send_partial_kv_response_once(B_PARTIAL_WDATA);
    cycle_count = 0;
    while (!b_partial_write_seen_r && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!b_partial_write_seen_r) $fatal(1, "B1 partial KV SRAM write was not observed");
    query_kv_tables_once(B_REF_TOKEN_ID, B_REF_POS);
    if (!kv_b_partial_seen_r) $fatal(1, "B group KV table lookup did not return partial-ready state");
    cycle_count = 0;
    while (!issue_b1_seen_r && (cycle_count < 12)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!issue_b1_seen_r) $fatal(1, "B1 partial-ready issue was not observed");
    cycle_count = 0;
    while (!wb_b1_seen_r && (cycle_count < 40)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!wb_b1_seen_r) $fatal(1, "B1 partial writeback payload was not observed");
    if (tree_busy !== 1'b1) $fatal(1, "B1 tree control should stay busy while waiting for full KV");
    cycle_count = 0;
    while (!recompute_resp_ready && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!recompute_resp_ready) $fatal(1, "B1 recompute response channel was not ready for full");
    send_full_kv_response_once(B_FULL_WDATA);
    cycle_count = 0;
    while (!b_full_write_seen_r && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!b_full_write_seen_r) $fatal(1, "B1 full KV SRAM write was not observed");
    query_kv_tables_once(B_REF_TOKEN_ID, B_REF_POS);
    if (!kv_b_full_seen_r) $fatal(1, "B group KV table lookup did not return full-ready state");
    cycle_count = 0;
    while ((tree_busy !== 1'b0) && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (tree_busy !== 1'b0) $fatal(1, "B1 tree control did not return to idle after full KV");
    ...
    if (tree_busy !== 1'b1) $fatal(1, "B2 tree control did not enter busy");
    drive_candidate_window_once(B2_PARENT_NODE_ID, B2_TOKEN_ID, B_REF_TOKEN_ID,
                                B_REF_POS, B2_CONF);
    cycle_count = 0;
    while (!pred_b2_seen_r && (cycle_count < 8)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!pred_b2_seen_r) $fatal(1, "B2 prediction-to-tree handshake was not observed");
    cycle_count = 0;
    while (!issue_b2_seen_r && (cycle_count < 12)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!issue_b2_seen_r) $fatal(1, "B2 full-ready reuse issue was not observed");
    cycle_count = 0;
    while (!wb_b2_seen_r && (cycle_count < 40)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (!wb_b2_seen_r) $fatal(1, "B2 writeback payload was not observed");
    cycle_count = 0;
    while ((tree_busy !== 1'b0) && (cycle_count < 16)) begin
        @(posedge clk); #1; cycle_count = cycle_count + 1;
    end
    if (tree_busy !== 1'b0) $fatal(1, "B2 tree control did not return to idle");
    if (recompute_req_count_b2_r != 0) $fatal(1, "B2 should not emit recompute request on full-ready reuse");

    if (recompute_req_count_a1_r != 1) $fatal(1, "A1 recompute request count mismatch");
    if (recompute_req_count_b1_r != 1) $fatal(1, "B1 recompute request count mismatch");
    if (recompute_done_count_r != 2) $fatal(1, "expected 2 recompute completion pulses across A1/B1");
    if (transform_read_count_r != 4) $fatal(1, "expected 4 transform reads across A1/A2/B1/B2");
    if (tree_error_flag !== 1'b0) $fatal(1, "tree control error flag should stay low");

    $display("tb_control_chip_stage2_recompute_mixed_fullready_reuse_min PASS");
    $finish;
end
