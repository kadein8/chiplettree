...
module tb_comparator_async_multi_depth_stale_drop;
...
task expect_reduction_result;
    begin
        ...
        if (!accepted_prefix_valid ||
            ...
            $fatal(1, "comparator reduction result mismatch before stale-drop check");
        end
    end
endtask

task expect_stale_result_dropped;
    integer wait_i;
    begin
        #1;
        if (!accepted_prefix_valid ||
            ...
            $fatal(1, "stale result should not perturb accepted prefix or live branch mask");
        end
        ...
        for (wait_i = 0; wait_i < 4; wait_i = wait_i + 1) begin
            ...
            if (prefetch_enq_valid) begin
                $fatal(1, "stale result should not create spontaneous AGU work");
            end
        end
    end
endtask
...
task issue_survivor_scalar_and_complete;
    ...
    begin
        ...
        if (!tree_in_ready ||
            ...
            $fatal(1, "survivor branch should still dispatch after stale drop");
        end
        @(posedge clk);
        #1;
        tree_in_valid = 1'b0;
...
task expect_killed_branch_still_blocked;
    input [`BRANCH_ID_W-1:0] branch_id_in;
    input [`NODE_ID_W-1:0] node_id_in;
    begin
        @(negedge clk);
        ...
        if (tree_in_ready || prefetch_enq_valid) begin
            $fatal(1, "killed branch should stay blocked after stale drop");
        end
        @(posedge clk);
        #1;
        tree_in_valid = 1'b0;
...
initial begin
    ...
    expect_stale_result_dropped();

    issue_survivor_scalar_and_complete(BRANCH_ABC1, NODE_C1);
    expect_killed_branch_still_blocked(BRANCH_ABC2, NODE_C2);

    $display("tb_comparator_async_multi_depth_stale_drop PASS");
    $finish;
end
