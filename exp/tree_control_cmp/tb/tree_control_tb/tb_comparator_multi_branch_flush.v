...
module tb_comparator_multi_branch_flush;
...
reg first_reclaim_seen_victim0_r;
reg first_reclaim_seen_victim1_r;
...
task lookup_expect;
    ...
    begin
        ...
        if (!lookup_resp_valid ||
            (lookup_resp_req_id != req_id) ||
            (lookup_resp_hit != expect_hit)) begin
            $fatal(1, "%0s: lookup response mismatch", scenario_name);
        end
        if (expect_hit) begin
            if ((lookup_resp_branch_mask != expect_branch_mask) ||
                (lookup_resp_state == {`TOKEN_STATE_W{1'b0}})) begin
                $fatal(1, "%0s: lookup hit payload mismatch", scenario_name);
            end
        end else if (lookup_resp_state != {`TOKEN_STATE_W{1'b0}}) begin
            $fatal(1, "%0s: lookup miss should return invalid state", scenario_name);
        end
        @(posedge clk);
        #1;
        clear_lookup();
...
task preload_prefetch_queue_victims_survivor;
    ...
    begin
        ...
        u_prefetch_queue.entry_count_r = 3;
    end
endtask
...
task expect_flush_bridge_active;
    ...
    begin
        ...
        if (!prefetch_flush_valid ||
            ...
            (token_flush_branch_mask != expect_branch_mask) ||
            (prefetch_flush_node_mask != expect_node_mask) ||
            (free_list_flush_node_mask != expect_node_mask) ||
            (token_flush_node_mask != expect_node_mask)) begin
            $fatal(1, "AGU should forward identical multi-branch masks to all flush consumers");
        end
    end
endtask

task expect_two_reclaim_handoffs;
    integer wait_i;
    reg seen_victim0;
    reg seen_victim1;
    begin
        seen_victim0 = first_reclaim_seen_victim0_r;
        seen_victim1 = first_reclaim_seen_victim1_r;
        for (wait_i = 0; wait_i < 40; wait_i = wait_i + 1) begin
            @(negedge clk);
            #1;
            if (flush_reclaim_valid) begin
                if ((flush_reclaim_sram_id == victim0_sram_id_r) &&
                    ...
                    if (seen_victim0) $fatal(1, "duplicate victim0 reclaim handoff");
                    seen_victim0 = 1'b1;
                    ...
                    if (seen_victim1) $fatal(1, "duplicate victim1 reclaim handoff");
                    seen_victim1 = 1'b1;
                end else begin
                    $fatal(1, "unexpected reclaim handoff payload");
                end
                if (seen_victim0 && seen_victim1) begin
                    disable expect_two_reclaim_handoffs;
                end
            end
        end
        if (!seen_victim0 || !seen_victim1) begin
            $fatal(1, "free_list should serialize reclaim handoffs for both victims");
        end
    end
endtask
    ...
    begin
        for (flat_i = 0; flat_i < TOTAL_BANKS; flat_i = flat_i + 1) begin
            for (bit_i = 0; bit_i < `SUBBANK_NUM_PER_BANK; bit_i = bit_i + 1) begin
                ...
                if (!victim0_hit && !victim1_hit && !survivor_hit) begin
...
initial begin
    ...
    first_reclaim_seen_victim0_r = 1'b0;
    first_reclaim_seen_victim1_r = 1'b0;
    ...
    issue_req_expect_grant("survivor_alloc", TEST_REQ_ID, SURVIVOR_BRANCH_ID, SURVIVOR_NODE_ID, 3'd3);

    lookup_expect("victim0_before_flush", TEST_REQ_ID, VICTIM0_TOKEN_ID, VICTIM0_POSITION_ID, 1'b1,
                  branch_onehot_mask(VICTIM0_BRANCH_ID));
    lookup_expect("victim1_before_flush", TEST_REQ_ID, VICTIM1_TOKEN_ID, VICTIM1_POSITION_ID, 1'b1,
                  branch_onehot_mask(VICTIM1_BRANCH_ID));
    lookup_expect("survivor_before_flush", TEST_REQ_ID, SURVIVOR_TOKEN_ID, SURVIVOR_POSITION_ID, 1'b1,
    ...
    expect_range_occupied("survivor_before_flush", query_bitmap_r, survivor_subbank_start_r, survivor_group_len_r);
    ...
    if (u_prefetch_queue.entry_count_r != 1 ||
        ...
        (u_prefetch_queue.entry_node_id_r[0] != SURVIVOR_NODE_ID)) begin
        $fatal(1, "prefetch_queue should keep only the survivor after multi-branch flush");
    end
    ...
    query_bank_bitmap(survivor_sram_id_r, survivor_bank_id_r);
    expect_range_occupied("survivor_after_flush", query_bitmap_r, survivor_subbank_start_r, survivor_group_len_r);
    ...
    expect_free_list_range_state("victim1_free_list_after_flush", victim1_sram_id_r, victim1_bank_id_r,
                                 ...
                                 {`REQ_ID_W{1'b0}}, {`BRANCH_ID_W{1'b0}}, {`NODE_ID_W{1'b0}});
    expect_free_list_range_state("survivor_free_list_after_flush", survivor_sram_id_r, survivor_bank_id_r,
                                 survivor_subbank_start_r, survivor_group_len_r, 1'b0,
                                 TEST_REQ_ID, SURVIVOR_BRANCH_ID, SURVIVOR_NODE_ID);

    lookup_expect("victim0_lookup_after_flush", TEST_REQ_ID, VICTIM0_TOKEN_ID, VICTIM0_POSITION_ID, 1'b0,
                  {`BRANCH_MASK_W{1'b0}});
    lookup_expect("victim1_lookup_after_flush", TEST_REQ_ID, VICTIM1_TOKEN_ID, VICTIM1_POSITION_ID, 1'b0,
                  {`BRANCH_MASK_W{1'b0}});
    lookup_expect("survivor_lookup_after_flush", TEST_REQ_ID, SURVIVOR_TOKEN_ID, SURVIVOR_POSITION_ID, 1'b1,
                  branch_onehot_mask(SURVIVOR_BRANCH_ID));
    ...
    lookup_expect("victim0_lookup_stays_miss", TEST_REQ_ID, VICTIM0_TOKEN_ID, VICTIM0_POSITION_ID, 1'b0,
                  {`BRANCH_MASK_W{1'b0}});
    lookup_expect("survivor_lookup_stays_hit", TEST_REQ_ID, SURVIVOR_TOKEN_ID, SURVIVOR_POSITION_ID, 1'b1,
                  branch_onehot_mask(SURVIVOR_BRANCH_ID));
    ...
    $display("tb_comparator_multi_branch_flush PASS");
    $finish;
end
