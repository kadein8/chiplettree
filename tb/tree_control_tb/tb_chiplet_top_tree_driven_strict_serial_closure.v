...
module tb_control_chip_tree_driven_strict_serial_closure;

localparam integer CFG_W = 32;
localparam integer TREE_COUNT = 5;
localparam [CFG_W-1:0] TREE0_CFG_DATA = 32'h04d0_0000;
localparam [CFG_W-1:0] TREE1_CFG_DATA = 32'h04d0_0001;
localparam [CFG_W-1:0] TREE2_CFG_DATA = 32'h04d0_0002;
localparam [CFG_W-1:0] TREE3_CFG_DATA = 32'h04d0_0003;
localparam [CFG_W-1:0] TREE4_CFG_DATA = 32'h04d0_0004;

localparam integer ROLE_CLEAN = 0;
localparam integer ROLE_SINGLE_FLUSH = 1;
localparam integer ROLE_FLUSH_CONTINUE = 2;
localparam integer ROLE_STALE_DROP = 3;
localparam integer ROLE_MULTI_FLUSH = 4;

localparam integer BUSY_ASSERT_TIMEOUT_CYCLES = 24;
localparam integer MODE_ASSERT_TIMEOUT_CYCLES = 12;
localparam integer TREE_VERIFY_TIMEOUT_CYCLES = 320;
...
function integer tree_role;
    input integer tree_idx;
    begin
        case (tree_idx)
            0: tree_role = ROLE_CLEAN;
            1: tree_role = ROLE_SINGLE_FLUSH;
            2: tree_role = ROLE_FLUSH_CONTINUE;
            3: tree_role = ROLE_STALE_DROP;
            4: tree_role = ROLE_MULTI_FLUSH;
            default: tree_role = ROLE_CLEAN;
        endcase
    end
endfunction
...
task automatic expect_tree_driven_mode_and_descriptor;
    ...
    begin
        ...
        for (cycle_i = 0; cycle_i < MODE_ASSERT_TIMEOUT_CYCLES;
            ...
            @(posedge clk);
            #1;
            if (u_control_chip.scenario_tree_driven_strict_serial_mode_r &&
        ...
        if (!seen_mode) begin
            $fatal(1,
                   "tree%0d: expected 4'hd tree-driven mode and descriptor id %0d",
                   tree_idx,
                   cfg_word[7:0]);
        end
    end
...
task automatic wait_for_tree_runtime_summary;
    input integer tree_idx;
    input integer role_i;
        ...
        case (role_i)
            ROLE_CLEAN: begin
                if (observed_effective_flush_count != 0) begin
                    $fatal(1, "tree%0d: clean tree should close with zero flush",
                           tree_idx);
                end
                if (!seen_concurrency_before_first_flush) begin
                    $fatal(1,
                           "tree%0d: clean tree should still show concurrency",
                           tree_idx);
                end
                clean_tree_cov_r = 1'b1;
            end

            ROLE_SINGLE_FLUSH: begin
                if (observed_effective_flush_count != 1) begin
                    $fatal(1,
                           "tree%0d: single-flush tree should close with one effective flush",
                           tree_idx);
                end
                if (!seen_concurrency_before_first_flush) begin
                    $fatal(1,
                           "tree%0d: single-flush tree should show concurrency before flush",
                           tree_idx);
                end
                single_flush_cov_r = 1'b1;
            end

            ROLE_FLUSH_CONTINUE: begin
                if (observed_effective_flush_count < 1) begin
                    $fatal(1,
                           "tree%0d: flush-then-continue tree should flush at least once",
                           tree_idx);
                end
                if (!seen_concurrency_before_first_flush ||
                    !seen_survivor_after_flush) begin
                    $fatal(1,
                           "tree%0d: flush-then-continue tree should show preflush concurrency and postflush survivor continuation",
                           tree_idx);
                end
                survivor_continue_cov_r = 1'b1;
            end

            ROLE_STALE_DROP: begin
                if (observed_effective_flush_count < 1) begin
                    $fatal(1,
                           "tree%0d: stale-drop tree should flush at least once",
                           tree_idx);
                end
                if (!seen_stale_drop || !seen_survivor_after_flush) begin
                    $fatal(1,
                           "tree%0d: stale-drop tree should show stale late-result drop and survivor continuation",
                           tree_idx);
                end
            ...
            ROLE_MULTI_FLUSH: begin
                if (observed_effective_flush_count < 2) begin
                    $fatal(1,
                           "tree%0d: multi-flush tree should close with at least two effective flushes",
                           tree_idx);
                end
                if (!seen_concurrency_before_first_flush ||
                    !seen_concurrency_after_flush ||
                    !seen_survivor_after_flush) begin
                    $fatal(1,
                           "tree%0d: multi-flush tree should show concurrency before and after flush plus survivor continuation",
                           tree_idx);
                end
            ...
            default: begin
                $fatal(1, "tree%0d: invalid role %0d", tree_idx, role_i);
            end
        endcase
    ...
    begin
        ...
        for (cycle_i = 0; cycle_i < WRITEBACK_TIMEOUT_CYCLES;
            ...
            if (seen_writeback && !busy) begin
                if (effective_flush_count_i > 0) begin
                    if (flush_pulse_total != frozen_flush_count) begin
                        $fatal(1,
    ...
    begin
        for (cycle_i = 0; cycle_i < QUIET_GAP_CYCLES; cycle_i = cycle_i + 1) begin
            ...
            if ((flush_pulse_total != exact_flush_count) ||
                ...
                $fatal(1,
                       "tree%0d: observed new flush or writeback activity in idle gap",
                       tree_idx);
            end
            ...
            if (u_control_chip.u_compute_module_bounded_stub.survivor_read_done ||
                ...
                |u_control_chip.rc_mem_req_write) begin
                $fatal(1,
                       "tree%0d: expected no residual bounded activity in idle gap",
                       tree_idx);
            end
        end
    end
...
initial begin
    ...
    if (!clean_tree_cov_r) begin
        $fatal(1, "whole_run: expected at least one no-flush closure");
    end

    if (!single_flush_cov_r) begin
        $fatal(1, "whole_run: expected at least one single-effective-flush closure");
    end

    if (!multi_flush_cov_r) begin
        $fatal(1, "whole_run: expected at least one multi-effective-flush closure");
    end
    ...
    if (busy || error_flag) begin
        $fatal(1,
               "whole_run: control_chip should end the bounded strict-serial tree-driven run without error");
    end

    $display("tb_control_chip_tree_driven_strict_serial_closure PASS");
    $finish;
end
