...
module tb_control_chip_stage2_prediction_tree_transform_writeback_min;
...
localparam [`TOKEN_ID_W-1:0] SEL_TOKEN_ID = 16'h3303;
localparam [`TOKEN_ID_W-1:0] SEL_REF_TOKEN_ID = 16'h7337;
localparam [`POSITION_ID_W-1:0] SEL_REF_POS = 12'h023;
localparam [CONF_W-1:0] SEL_CONF = 8'd200;
...
task drive_candidate_window_once;
    begin
        ...
        draft_referenced_token_id[(0*`TOKEN_ID_W) +: `TOKEN_ID_W] = 16'h7202;
...
initial begin
    ...
    cycle_count = 0;
    while (!issue_seen_r && (cycle_count < 8)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (!issue_seen_r) begin
        $fatal(1, "tree issue metadata did not match selected prediction");
    end
    ...
    $display("tb_control_chip_stage2_prediction_tree_transform_writeback_min PASS");
    $finish;
end
