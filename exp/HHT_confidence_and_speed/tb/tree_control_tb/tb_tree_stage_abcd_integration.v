...
module tb_tree_stage_abcd_integration ();
...
localparam [`BRANCH_MASK_W-1:0] BRANCH_MASK_0 = {{(`BRANCH_MASK_W-1){1'b0}}, 1'b1};
localparam [`REQ_ID_W-1:0] PREP_REQ_ID = 4'ha;
localparam [`REQ_ID_W-1:0] PE_READ_REQ_ID = 4'hb;
localparam [`REQ_ID_W-1:0] PE_BLOCKED_REQ_ID = 4'hc;
localparam [`PE_MASK_W-1:0] PE_MASK_TEST = 16'h0f0f;
localparam [`SRAM_WDATA_W-1:0] TEST_DATA =
    128'h13579bdf_2468ace0_fedcba98_76543210;
...
reg [`SRAM_ID_W-1:0] observed_sram_id_r;
reg [`BANK_ID_W-1:0] observed_bank_id_r;
reg [`SUBBANK_ID_W-1:0] observed_subbank_start_r;
reg [`KV_GROUP_LEN_W-1:0] observed_group_len_r;
...
tree_analyze u_tree_analyze (
    ...
    .req_id(req_id), .src_prefix_slot_valid(src_prefix_slot_valid),
    ...
    .prefix_req_id(prefix_req_id), .prefix_node_valid(prefix_node_valid),
    .prefix_node_id(prefix_node_id), .prefix_parent_node_id(prefix_parent_node_id),
    .prefix_token_id(prefix_token_id), .prefix_position_id(prefix_position_id),
    .prefix_layer_id(prefix_layer_id), .prefix_is_last(prefix_is_last),
    .frontier_valid(frontier_valid), .frontier_ready(frontier_ready),
    .frontier_req_id(frontier_req_id), .frontier_level_id(frontier_level_id),
    .frontier_slot_valid(frontier_slot_valid), .frontier_node_id(frontier_node_id),
...
agu u_agu (
    .clk(clk), .rst_n(rst_n),
    .tree_in_valid(1'b0), .tree_in_ready(),
    ...
    .cand_resp_valid(cand_resp_valid), .cand_resp_grant(cand_resp_grant),
    .cand_resp_req_id(cand_resp_req_id), .cand_resp_sram_id(cand_resp_sram_id),
    ...
    .alloc_resp_valid(alloc_resp_valid), .alloc_resp_grant(alloc_resp_grant),
    .alloc_resp_req_id(alloc_resp_req_id), .alloc_resp_sram_id(alloc_resp_sram_id),
    ...
    .prefix_norm_valid(), .prefix_norm_req_id(), .prefix_norm_node_valid(),
    .prefix_norm_node_id(), .prefix_norm_parent_node_id(),
    .prefix_norm_token_id(), .prefix_norm_position_id(), .prefix_norm_layer_id(),
    .prefix_norm_is_last(), .prefix_norm_is_shared(),
    .frontier_norm_valid(), .frontier_norm_req_id(), .frontier_norm_level_id(),
    .frontier_norm_slot_valid(), .frontier_norm_node_id(),
    .frontier_norm_parent_node_id(), .frontier_norm_token_id(),
    .frontier_norm_position_id(), .frontier_norm_size_subbank(),
    .frontier_norm_slot_shared(),
    .alloc_cand_valid(), .alloc_cand_req_id(),
    ...
    .token_wr_valid(token_wr_valid), .token_wr_req_id(token_wr_req_id),
...
prefetch_queue u_prefetch_queue (
    .clk(clk), .rst_n(rst_n),
    .enq_valid(prefetch_enq_valid), .enq_ready(prefetch_enq_ready),
    .enq_req_id(prefetch_enq_req_id), .enq_branch_id(prefetch_enq_branch_id),
    .enq_node_id(prefetch_enq_node_id), .enq_layer_id(prefetch_enq_layer_id),
    .enq_size_subbank(prefetch_enq_size_subbank), .enq_shared(prefetch_enq_shared),
    .flush_valid(1'b0), .flush_req_id({`REQ_ID_W{1'b0}}),
    .flush_branch_mask({`BRANCH_MASK_W{1'b0}}),
    .flush_node_mask({`NODE_MASK_W{1'b0}}),
    .deq_valid(prefetch_deq_valid), .deq_ready(prefetch_deq_ready),
    .deq_req_id(prefetch_deq_req_id), .deq_branch_id(prefetch_deq_branch_id),
    .deq_node_id(prefetch_deq_node_id), .deq_layer_id(prefetch_deq_layer_id),
    .deq_size_subbank(prefetch_deq_size_subbank), .deq_shared(prefetch_deq_shared)
);

free_list u_free_list (
    .clk(clk), .rst_n(rst_n),
    .cand_req_valid(prefetch_deq_valid), .cand_req_ready(prefetch_deq_ready),
    ...
    .flush_branch_mask({`BRANCH_MASK_W{1'b0}}),
    .flush_node_mask({`NODE_MASK_W{1'b0}}),
    .release_valid(1'b0), .release_sram_id({`SRAM_ID_W{1'b0}}),
...
bank_state_table u_bank_state_table (
    .clk(clk), .rst_n(rst_n),
    .cand_valid(alloc_cand_valid), .cand_ready(),
    .cand_req_id(alloc_cand_req_id), .cand_branch_id(alloc_cand_branch_id),
    .cand_node_id(alloc_cand_node_id), .cand_size_subbank(alloc_cand_size_subbank),
    .cand_shared(alloc_cand_shared), .cand_sram_id(alloc_cand_sram_id),
    ...
    .commit_valid(1'b0), .commit_req_id({`REQ_ID_W{1'b0}}),
    .commit_sram_id({`SRAM_ID_W{1'b0}}), .commit_bank_id({`BANK_ID_W{1'b0}}),
    ...
    .flush_branch_mask({`BRANCH_MASK_W{1'b0}}),
    .flush_node_mask({`NODE_MASK_W{1'b0}}),
    .reclaim_valid(1'b0), .reclaim_sram_id({`SRAM_ID_W{1'b0}}),
    .reclaim_bank_id({`BANK_ID_W{1'b0}}),
    .reclaim_subbank_start({`SUBBANK_ID_W{1'b0}}),
    .reclaim_group_len({`KV_GROUP_LEN_W{1'b0}}),
    .query_valid(1'b0), .query_sram_id({`SRAM_ID_W{1'b0}}),
    .query_bank_id({`BANK_ID_W{1'b0}}), .query_resp_valid(),
    .query_resp_occ_bitmap(), .query_resp_state(),
    .query_resp_branch_mask(), .query_resp_refcnt()
);

token_register u_token_register (
    .clk(clk), .rst_n(rst_n),
    .wr_valid(token_wr_valid), .wr_ready(), .wr_index(token_wr_index_r),
    .wr_req_id(token_wr_req_id), .wr_token_id(token_wr_token_id),
    .wr_position_id(token_wr_position_id), .wr_node_id(token_wr_node_id),
    .wr_branch_id(token_wr_branch_id), .wr_sram_id(token_wr_sram_id),
    .wr_bank_id(token_wr_bank_id), .wr_subbank_start(token_wr_subbank_start),
    .wr_group_len(token_wr_group_len), .wr_branch_mask(token_wr_branch_mask),
    .wr_is_shared(token_wr_is_shared),
    .lookup_valid(lookup_valid), .lookup_ready(lookup_ready),
    .lookup_req_id(lookup_req_id), .lookup_token_id(lookup_token_id),
    .lookup_position_id(lookup_position_id),
    .lookup_resp_valid(lookup_resp_valid), .lookup_resp_hit(lookup_resp_hit),
...
.lookup_resp_state(lookup_resp_state),
    .commit_valid(1'b0), .commit_index({`TOKEN_REG_INDEX_W{1'b0}}),
    ...
    .flush_branch_mask({`BRANCH_MASK_W{1'b0}}),
    .flush_node_mask({`NODE_MASK_W{1'b0}}),
    .entry_count(), .error_flag()
);

request_controller u_request_controller (
    .clk(clk), .rst_n(rst_n),
    .req_in_valid(pe_req_valid), .req_in_ready(pe_req_ready),
    .req_in_write(pe_req_write), .req_in_addr(pe_req_addr),
    .req_in_wdata(pe_req_wdata), .req_in_req_id(pe_req_req_id),
    .req_in_pe_mask(pe_req_pe_mask), .req_in_priority(pe_req_priority),
    .req_in_bank_id(pe_req_bank_id), .req_in_subbank_id(pe_req_subbank_id),
    .mem_req_valid(rc_mem_req_valid), .mem_req_ready(rc_mem_req_ready),
    .mem_req_write(rc_mem_req_write), .mem_req_addr(rc_mem_req_addr),
    .mem_req_wdata(rc_mem_req_wdata), .mem_req_id(rc_mem_req_id),
    .mem_resp_valid(sram_mem_resp_valid),
    .mem_resp_rdata(sram_mem_resp_rdata),
    .mem_resp_id(sram_mem_resp_id), .mem_resp_last(sram_mem_resp_last),
    .resp_out_valid(pe_resp_valid), .resp_out_ready(pe_resp_ready),
    .resp_out_rdata(pe_resp_rdata), .resp_out_req_id(pe_resp_req_id),
    .resp_out_pe_mask(pe_resp_pe_mask), .resp_out_last(pe_resp_last)
);

sram_subsystem u_sram_subsystem (
    .clk(clk), .rst_n(rst_n),
    .mem_req_valid(sram_mem_req_valid), .mem_req_ready(sram_mem_req_ready),
    .mem_req_write(sram_mem_req_write), .mem_req_addr(sram_mem_req_addr),
    .mem_req_wdata(sram_mem_req_wdata), .mem_req_id(sram_mem_req_id),
    .mem_resp_valid(sram_mem_resp_valid), .mem_resp_rdata(sram_mem_resp_rdata),
    .mem_resp_id(sram_mem_resp_id), .mem_resp_last(sram_mem_resp_last)
);
...
task clear_tree_request;
    begin
        ...
        src_frontier_slot_valid = {(`TREE_MAX_FRONTIER_LEVELS*`TREE_FRONTIER_SLOTS){1'b0}};
        src_frontier_node_id = {(`TREE_MAX_FRONTIER_LEVELS*`TREE_FRONTIER_SLOTS*`NODE_ID_W){1'b0}};
        src_frontier_parent_node_id = {(`TREE_MAX_FRONTIER_LEVELS*`TREE_FRONTIER_SLOTS*`NODE_ID_W){1'b0}};
    end
endtask
        ...
        for (wait_i = 0; wait_i < 12; wait_i = wait_i + 1) begin
            ...
            if (token_wr_valid) begin
                if (token_wr_req_id != TEST_REQ_ID ||
                    ...
                    $fatal(1, "ABCD token write mismatch");
                end
                if (!alloc_resp_valid ||
                    !alloc_resp_grant ||
                    ...
                    $fatal(1, "ABCD bank_state_table response mismatch");
                end
                observed_sram_id_r = token_wr_sram_id;
                observed_bank_id_r = token_wr_bank_id;
                observed_subbank_start_r = token_wr_subbank_start;
                observed_group_len_r = token_wr_group_len;
                seen = 1'b1;
                disable wait_for_alloc_and_token;
            end
        end
        if (!seen) begin
            $fatal(1, "ABCD integration did not reach alloc/token side effects");
        end
    end
endtask
    ...
    begin
        ...
        for (wait_i = 0; wait_i < 4; wait_i = wait_i + 1) begin
            ...
            if (lookup_resp_valid) begin
                seen = 1'b1;
                if (!lookup_resp_hit || lookup_resp_req_id != TEST_REQ_ID) begin
                    $fatal(1, "ABCD token lookup response mismatch");
                end
                disable wait_for_lookup_hit;
            end
        end
        if (!seen) begin
            $fatal(1, "ABCD token lookup did not return");
        end
    end
endtask
...
task token_sram_prepare_write;
    ...
    begin
        ...
        if (!sram_mem_req_ready[SRAM_LANE]) begin
            $fatal(1, "ABCD token/SRAM preparation write was not accepted");
        end
        @(posedge clk);
        #1;
        clear_prep_req();
...
task pe_stub_drive_read;
    ...
    begin
        ...
        if (!pe_req_ready) begin
            $fatal(1, "ABCD PE-stub request should be accepted while controller is idle");
        end
        @(posedge clk);
        #1;
        clear_pe_req();
...
task wait_for_pe_response;
    ...
    begin
        ...
        if (!seen) begin
            $fatal(1, "ABCD PE-stub did not receive request_controller response");
        end
    end
endtask
...
initial begin
    ...
    observed_sram_id_r = {`SRAM_ID_W{1'b0}};
    observed_bank_id_r = {`BANK_ID_W{1'b0}};
    observed_subbank_start_r = {`SUBBANK_ID_W{1'b0}};
    observed_group_len_r = {`KV_GROUP_LEN_W{1'b0}};
    ...
    wait_for_alloc_and_token(NODE_A, BRANCH_0, BRANCH_MASK_ALL, 1'b1);

    wait_for_alloc_and_token(NODE_C1, BRANCH_0, BRANCH_MASK_0, 1'b0);
    ...
    pe_req_wdata = {`SRAM_WDATA_W{1'b0}};
    pe_req_req_id = PE_BLOCKED_REQ_ID;
    pe_req_pe_mask = 16'h0001;
    pe_req_bank_id = lookup_resp_bank_id;
    pe_req_subbank_id = lookup_resp_subbank_start;
    #1;
    if (pe_req_ready) begin
        $fatal(1, "ABCD busy request_controller accepted a second PE request");
    end
    clear_pe_req();
    ...
    if (pe_resp_rdata[0 +: `SRAM_RDATA_W] != TEST_DATA ||
        ...
        $fatal(1, "ABCD PE-stub response data or metadata mismatch");
    end

    @(posedge clk);
    #1;
    if (!pe_resp_valid[0]) begin
        $fatal(1, "ABCD request_controller response should hold until PE ready");
    end
    ...
    pe_resp_ready = {`MEM_REQ_LANES{1'b0}};
    if (pe_resp_valid[0]) begin
        $fatal(1, "ABCD request_controller response did not clear after PE handshake");
    end

    $display("tb_tree_stage_abcd_integration PASS");
    $finish;
end
