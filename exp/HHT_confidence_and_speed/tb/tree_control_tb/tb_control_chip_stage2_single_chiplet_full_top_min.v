...
module tb_control_chip_stage2_single_chiplet_full_top_min;
...
localparam [`TOKEN_ID_W-1:0] SEL_TOKEN_ID = 16'hca21;
localparam [`TOKEN_ID_W-1:0] SEL_REF_TOKEN_ID = 16'h8c21;
...
task expect_ready_mask;
    ...
    begin
        ...
        if ((hht_cand_ready !== expected_hht_ready) ||
            ...
            $fatal(1, "single-chiplet top ready mask mismatch");
        end
    end
endtask
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (tree_window_valid &&
            ...
            (tree_window_referenced_position[0 +: `POSITION_ID_W] == SEL_REF_POS) &&
            (tree_window_confidence[0 +: CONF_W] == SEL_CONF)) begin
...
initial begin
    ...
    if (busy !== 1'b1) begin
        $fatal(1, "single-chiplet top did not enter busy");
    end
    ...
    if (!tree_window_seen_r) begin
        $fatal(1, "single-chiplet tree window was not observed");
    end
    ...
    if (!recompute_req_seen_r) begin
        $fatal(1, "single-chiplet recompute request was not observed");
    end
    if (!debug_stale_hit) begin
        $fatal(1, "single-chiplet stale hit was not observed");
    end
    if (!debug_recompute_busy) begin
        $fatal(1, "single-chiplet recompute control did not enter busy");
    end
    ...
    if (!recompute_resp_ready) begin
        $fatal(1, "single-chiplet recompute response channel was not ready");
    end
    ...
    if (!recompute_done_seen_r) begin
        $fatal(1, "single-chiplet recompute completion pulse was not observed");
    end

    query_kv_tables_once();
    if (!kv_full_seen_r) begin
        $fatal(1, "single-chiplet KV full-ready lookup was not observed");
    end
    ...
    if (!decoder_qkv_seen_r) begin
        $fatal(1, "single-chiplet decoder qkv stage was not observed");
    end
    if (!decoder_score_seen_r) begin
        $fatal(1, "single-chiplet decoder score stage was not observed");
    end
    if (!decoder_softmax_seen_r) begin
        $fatal(1, "single-chiplet decoder softmax stage was not observed");
    end
    if (!decoder_value_seen_r) begin
        $fatal(1, "single-chiplet decoder value stage was not observed");
    end
    if (!decoder_ffn_seen_r) begin
        $fatal(1, "single-chiplet decoder ffn stage was not observed");
    end
    if (!wb_seen_r) begin
        $fatal(1, "single-chiplet writeback payload was not observed");
    end
    if (!wb_done) begin
        $fatal(1, "single-chiplet writeback completion was not observed");
    end
    ...
    if (busy !== 1'b0) begin
        $fatal(1, "single-chiplet top did not return to idle");
    end
    if (error_flag !== 1'b0) begin
        $fatal(1, "single-chiplet error flag should stay low");
    end
    if (wb_error !== 1'b0) begin
        $fatal(1, "single-chiplet writeback error should stay low");
    end
    if (hbm_req_valid !== 1'b0) begin
        $fatal(1, "single-chiplet minimal path should not issue HBM request");
    end
    if (hbm_req_write !== 1'b0) begin
        $fatal(1, "single-chiplet minimal path HBM write flag should stay low");
    end
    if (hbm_req_addr != {`HBM_ADDR_W{1'b0}}) begin
        $fatal(1, "single-chiplet minimal path HBM address should stay zero");
    end
    if (hbm_req_wdata != {`HBM_DATA_W{1'b0}}) begin
        $fatal(1, "single-chiplet minimal path HBM data should stay zero");
    end
    if (hbm_req_id != {`REQ_ID_W{1'b0}}) begin
        $fatal(1, "single-chiplet minimal path HBM id should stay zero");
    end

    $display("tb_control_chip_stage2_single_chiplet_full_top_min PASS");
    $finish;
end
