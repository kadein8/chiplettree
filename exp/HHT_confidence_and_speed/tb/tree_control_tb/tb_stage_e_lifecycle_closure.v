...
module tb_stage_e_lifecycle_closure;
...
localparam [`REQ_ID_W-1:0] FILLER_REQ_ID = 4'h7;
localparam [`REQ_ID_W-1:0] FULL_REQ_ID = 4'h8;
...
localparam [`TOKEN_ID_W-1:0] VICTIM_TOKEN_ID = NODE_C2;
localparam [`POSITION_ID_W-1:0] VICTIM_POSITION_ID = NODE_C2;
localparam [`TOKEN_ID_W-1:0] SURVIVOR_TOKEN_ID = NODE_D1;
localparam [`POSITION_ID_W-1:0] SURVIVOR_POSITION_ID = NODE_D1;
...
task drive_deeper_accepted_result;
    begin
...
task drive_nonlive_short_branch_result;
    begin
        result_slot_valid = 4'b0001;
        result_req_id = TEST_REQ_ID;
        result_branch_id[(0*`BRANCH_ID_W) +: `BRANCH_ID_W] = BRANCH_ABC1;
        result_branch_epoch[(0*BRANCH_EPOCH_W) +: BRANCH_EPOCH_W] = 2'd0;
        result_private_depth[(0*PRIVATE_DEPTH_W) +: PRIVATE_DEPTH_W] = PRIVATE_DEPTH_W'(1);
...
task expect_prefix_state_c1;
    ...
    begin
        ...
        if (!accepted_prefix_valid ||
            ...
            (accepted_prefix_node_id[(0*`NODE_ID_W) +: `NODE_ID_W] != NODE_C1) ||
            (live_branch_mask != 4'b0011) ||
            (prune_branch_mask != expect_prune_mask)) begin
            $fatal(1, "%0s: C1 accepted-prefix state mismatch", scenario_name);
        end
    end
endtask

task expect_prefix_state_c1d1;
    input [255:0] scenario_name;
    input [`BRANCH_NUM-1:0] expect_prune_mask;
    begin
        #1;
        if (!accepted_prefix_valid ||
            (accepted_prefix_req_id != TEST_REQ_ID) ||
            (accepted_prefix_depth != PRIVATE_DEPTH_W'(2)) ||
            (accepted_prefix_node_id[(0*`NODE_ID_W) +: `NODE_ID_W] != NODE_C1) ||
            (accepted_prefix_node_id[(1*`NODE_ID_W) +: `NODE_ID_W] != NODE_D1) ||
            (live_branch_mask != 4'b0010) ||
            (prune_branch_mask != expect_prune_mask)) begin
            $fatal(1, "%0s: C1D1 accepted-prefix state mismatch", scenario_name);
        end
    end
endtask
...
task preload_prefetch_queue_victim_survivor;
    ...
    begin
        ...
        u_prefetch_queue.entry_branch_id_r[0] = BRANCH_ABC2;
        u_prefetch_queue.entry_node_id_r[0] = NODE_C2;
        ...
        u_prefetch_queue.entry_branch_id_r[1] = BRANCH_ABC1D1;
        u_prefetch_queue.entry_node_id_r[1] = NODE_D1;
...
task expect_preloaded_queue_state;
    begin
        ...
        if (u_prefetch_queue.entry_count_r != 2 ||
            ...
            (queue_deq_branch_id != BRANCH_ABC2) ||
            (queue_deq_node_id != NODE_C2)) begin
...
task expect_flush_bridge_active;
    begin
        ...
        if (!accepted_prefix_valid ||
            ...
            (accepted_prefix_node_id[(0*`NODE_ID_W) +: `NODE_ID_W] != NODE_C1) ||
            (accepted_prefix_node_id[(1*`NODE_ID_W) +: `NODE_ID_W] != NODE_D1) ||
            (live_branch_mask != 4'b0010)) begin
            $fatal(1, "accepted-prefix state should remain on the longest survivor during flush");
        end
...
task expect_prefetch_queue_victim_removed_survivor_kept;
    begin
        ...
        if (u_prefetch_queue.entry_count_r != 1 ||
            !u_prefetch_queue.entry_valid_r[0] ||
            (u_prefetch_queue.entry_req_id_r[0] != TEST_REQ_ID) ||
            (u_prefetch_queue.entry_branch_id_r[0] != BRANCH_ABC1D1) ||
            (u_prefetch_queue.entry_node_id_r[0] != NODE_D1) ||
            (u_prefetch_queue.entry_size_subbank_r[0] != `KV_GROUP_SIZE_SUBBANK) ||
...
initial begin
    ...
    issue_req_expect_grant("victim_alloc", TEST_REQ_ID,
                           BRANCH_ABC2, NODE_C2, 2'd1);
    issue_req_expect_grant("survivor_alloc", TEST_REQ_ID,
                           BRANCH_ABC1D1, NODE_D1, 2'd2);
    ...
    expect_free_list_range_state("survivor_free_list_before_flush",
                                 ...
                                 survivor_subbank_start_r, survivor_group_len_r,
                                 1'b0, TEST_REQ_ID,
                                 BRANCH_ABC1D1, NODE_D1);

    preload_active_branches();
    drive_first_discriminating_results();
    expect_prefix_state_c1("phase_a_shallow_accept", 4'b1100);

    @(posedge clk);
    #1;
    clear_comparator_inputs();

    drive_deeper_accepted_result();
    expect_prefix_state_c1d1("phase_b_deeper_accept", 4'b0001);
    expect_branch_blocked("phase_b_shorter_branch_blocked", BRANCH_ABC1, NODE_C1);
    expect_branch_blocked("phase_b_incompatible_branch_blocked", BRANCH_ABC2, NODE_C2);

    drive_nonlive_short_branch_result();
    expect_prefix_state_c1d1("phase_c_nonlive_result_ignored", 4'b0000);
    expect_no_spontaneous_issue("phase_c_no_spurious_issue", 4);
    ...
    issue_req_expect_deny("full_before_flush", FULL_REQ_ID, BRANCH_ABC2D2, NODE_D2);
    ...
    cmp_slot_node_id[`NODE_ID_W-1:0] = NODE_C2;
    cmp_slot_parent_node_id[`NODE_ID_W-1:0] = NODE_NONE;
    cmp_slot_branch_id[`BRANCH_ID_W-1:0] = BRANCH_ABC2;
    ...
    expect_branch_blocked("phase_e_shorter_branch_post_flush", BRANCH_ABC1, NODE_C1);
    expect_branch_blocked("phase_e_incompatible_branch_post_flush", BRANCH_ABC2, NODE_C2);

    issue_req_expect_grant("post_flush_survivor_continue", TEST_REQ_ID,
                           BRANCH_ABC1D1, NODE_D1, 2'd3);
    ...
    $display("tb_stage_e_lifecycle_closure PASS");
    $finish;
end
