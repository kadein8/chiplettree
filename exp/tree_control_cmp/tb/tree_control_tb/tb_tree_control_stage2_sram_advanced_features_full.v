...
module tb_tree_control_stage2_sram_advanced_features_full;
...
always @(posedge clk) begin
    #1;
    if (rst_n && measure_mem_issues_r) begin
            ...
            if (mem_req_valid[mon_i] && mem_req_ready[mon_i]) begin
                if (mem_req_write[mon_i]) begin
                    ...
                    if (mem_req_addr[(mon_i*`SRAM_ADDR_W) +: `SRAM_ADDR_W] == addr_a_r) begin
                        mem_write_issue_addr_a_count =
                            mem_write_issue_addr_a_count + 1;
                    end
                    if (mem_req_addr[(mon_i*`SRAM_ADDR_W) +: `SRAM_ADDR_W] == addr_b_r) begin
                        mem_write_issue_addr_b_count =
                            mem_write_issue_addr_b_count + 1;
                    end
                end else begin
                    mem_read_issue_total_count = mem_read_issue_total_count + 1;
                    if (mem_req_addr[(mon_i*`SRAM_ADDR_W) +: `SRAM_ADDR_W] == addr_a_r) begin
                        mem_read_issue_addr_a_count =
                            mem_read_issue_addr_a_count + 1;
                    end
                    if (mem_req_addr[(mon_i*`SRAM_ADDR_W) +: `SRAM_ADDR_W] == addr_b_r) begin
                        mem_read_issue_addr_b_count =
                            mem_read_issue_addr_b_count + 1;
                    end
                    if (mem_req_addr[(mon_i*`SRAM_ADDR_W) +: `SRAM_ADDR_W] == addr_c_r) begin
                        mem_read_issue_addr_c_count =
                            mem_read_issue_addr_c_count + 1;
                    end
                end
            end
        end
...
task wait_mdv_dual_group_response;
    reg seen;
    begin
        seen = 1'b0;
        for (wait_i = 0; wait_i < 20; wait_i = wait_i + 1) begin
            ...
            if (resp_out_valid == 4'b1111) begin
                seen = 1'b1;
                if (resp_out_rdata[(0*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != DATA_A ||
                    resp_out_rdata[(1*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != DATA_B ||
                    resp_out_rdata[(2*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != DATA_A ||
                    resp_out_rdata[(3*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != DATA_B ||
                    resp_out_req_id[(0*`REQ_ID_W) +: `REQ_ID_W] != MDV_A0_REQ_ID ||
                    resp_out_req_id[(1*`REQ_ID_W) +: `REQ_ID_W] != MDV_B0_REQ_ID ||
                    resp_out_req_id[(2*`REQ_ID_W) +: `REQ_ID_W] != MDV_A1_REQ_ID ||
                    resp_out_req_id[(3*`REQ_ID_W) +: `REQ_ID_W] != MDV_B1_REQ_ID ||
                    resp_out_pe_mask[(0*`PE_MASK_W) +: `PE_MASK_W] != MDV_A0_PE_MASK ||
                    resp_out_pe_mask[(1*`PE_MASK_W) +: `PE_MASK_W] != MDV_B0_PE_MASK ||
                    resp_out_pe_mask[(2*`PE_MASK_W) +: `PE_MASK_W] != MDV_A1_PE_MASK ||
                    resp_out_pe_mask[(3*`PE_MASK_W) +: `PE_MASK_W] != MDV_B1_PE_MASK ||
                    resp_out_last != 4'b1111) begin
                    $fatal(1, "mdv_dual_group: response payload mismatch");
                end
                release_response_mask(4'b1111);
                disable wait_mdv_dual_group_response;
            end
        end
        if (!seen) begin
            $fatal(1, "mdv_dual_group: response did not arrive");
        end
    end
endtask

task wait_write_after_read_group_bypass_response;
    reg seen;
    begin
        seen = 1'b0;
        for (wait_i = 0; wait_i < 20; wait_i = wait_i + 1) begin
            ...
            if (resp_out_valid == 4'b0011) begin
                seen = 1'b1;
                if (resp_out_rdata[(0*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != UPDATED_A ||
                    resp_out_rdata[(1*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != UPDATED_A ||
                    resp_out_req_id[(0*`REQ_ID_W) +: `REQ_ID_W] != WAR_A0_REQ_ID ||
                    resp_out_req_id[(1*`REQ_ID_W) +: `REQ_ID_W] != WAR_A1_REQ_ID ||
                    resp_out_pe_mask[(0*`PE_MASK_W) +: `PE_MASK_W] != WAR_A0_PE_MASK ||
                    resp_out_pe_mask[(1*`PE_MASK_W) +: `PE_MASK_W] != WAR_A1_PE_MASK ||
                    resp_out_last[1:0] != 2'b11) begin
                    $fatal(1, "write_after_read_group_bypass: response payload mismatch");
                end
                release_response_mask(4'b0011);
                disable wait_write_after_read_group_bypass_response;
            end
        end
        if (!seen) begin
            $fatal(1, "write_after_read_group_bypass: response did not arrive");
        end
    end
endtask

task wait_read_after_write_bypass_response;
    reg seen;
    begin
        seen = 1'b0;
        for (wait_i = 0; wait_i < 20; wait_i = wait_i + 1) begin
            @(posedge clk);
            #1;
            if (resp_out_valid == 4'b0101) begin
                seen = 1'b1;
                if (resp_out_rdata[(0*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != DATA_C ||
                    resp_out_rdata[(2*`SRAM_RDATA_W) +: `SRAM_RDATA_W] != UPDATED_B ||
                    resp_out_req_id[(0*`REQ_ID_W) +: `REQ_ID_W] != RAW_C_REQ_ID ||
                    resp_out_req_id[(2*`REQ_ID_W) +: `REQ_ID_W] != RAW_B_REQ_ID ||
                    resp_out_pe_mask[(0*`PE_MASK_W) +: `PE_MASK_W] != RAW_C_PE_MASK ||
                    resp_out_pe_mask[(2*`PE_MASK_W) +: `PE_MASK_W] != RAW_B_PE_MASK ||
                    !resp_out_last[0] ||
                    !resp_out_last[2]) begin
                    $fatal(1, "read_after_write_bypass: response payload mismatch");
                end
                release_response_mask(4'b0101);
                disable wait_read_after_write_bypass_response;
            end
        end
        if (!seen) begin
            $fatal(1, "read_after_write_bypass: response did not arrive");
        end
    end
endtask
...
initial begin
    ...
    issue_candidate_expect_grant(
        "shared_prefix_same_branch_reuse",
        4'hf,
        2'd0,
        4'd2,
        1'b1,
    ...
    if (u_bank_state_table.promoted_bitmap
        ...
        $fatal(1, "shared_prefix_same_branch_reuse: same branch reuse must not trigger promotion");
    end

    sample_query_bank("shared_prefix_same_branch_reuse_query",
                      shared_sram_r, shared_bank_r);
    if (sampled_query_state_r != BANK_STATE_SPEC ||
        sampled_query_branch_mask_r != BRANCH0_MASK ||
        sampled_query_refcnt_r != {{(`REFCNT_W-1){1'b0}}, 1'b1}) begin
        $fatal(1, "shared_prefix_same_branch_reuse_query: query state mismatch");
    end

    issue_candidate_expect_grant(
        "shared_prefix_cross_branch_reuse",
        4'hf,
        2'd1,
        4'd2,
        1'b1,
    ...
    if (!u_bank_state_table.promoted_bitmap
            [(shared_sram_r * `SRAM_BANK_NUM) + shared_bank_r][shared_subbank_r]) begin
        $fatal(1, "shared_prefix_cross_branch_reuse: promotion metadata missing");
    end

    sample_query_bank("shared_prefix_cross_branch_reuse_query",
                      shared_sram_r, shared_bank_r);
    if (sampled_query_state_r != BANK_STATE_SPEC ||
        sampled_query_branch_mask_r != (BRANCH0_MASK | BRANCH1_MASK) ||
        sampled_query_refcnt_r != {{(`REFCNT_W-2){1'b0}}, 2'b10}) begin
        $fatal(1, "shared_prefix_cross_branch_reuse_query: fan-out branch visibility mismatch");
    end

    if (u_free_list.entry_branch_mask
            [(shared_sram_r * `SRAM_BANK_NUM) + shared_bank_r][shared_subbank_r] !=
        (BRANCH0_MASK | BRANCH1_MASK)) begin
        $fatal(1, "shared_prefix_cross_branch_reuse: free_list branch mask must track real fan-out");
    end
    ...
    if (sampled_query_state_r != BANK_STATE_COMMITTED ||
        sampled_query_branch_mask_r != (BRANCH0_MASK | BRANCH1_MASK) ||
        sampled_query_refcnt_r != {{(`REFCNT_W-2){1'b0}}, 2'b10}) begin
        $fatal(1, "shared_prefix_after_commit: committed query mismatch");
    end
    ...
    wait_mdv_dual_group_response();
    measure_mem_issues_r = 1'b0;

    if (mem_read_issue_total_count != 2 ||
        mem_read_issue_addr_a_count != 1 ||
        mem_read_issue_addr_b_count != 1) begin
        $fatal(1, "mdv_dual_group: physical read issue count mismatch");
    end
    ...
    wait_write_after_read_group_bypass_response();
    measure_mem_issues_r = 1'b0;

    if (mem_read_issue_total_count != 0 ||
        mem_write_issue_total_count != 1 ||
        mem_write_issue_addr_a_count != 1) begin
        $fatal(1, "write_after_read_group_bypass: issue count mismatch");
    end
    ...
    wait_read_after_write_bypass_response();
    measure_mem_issues_r = 1'b0;

    if (mem_read_issue_total_count != 1 ||
        mem_write_issue_total_count != 1 ||
        mem_read_issue_addr_b_count != 0 ||
        mem_read_issue_addr_c_count != 1 ||
        mem_write_issue_addr_b_count != 1) begin
        $fatal(1, "read_after_write_bypass: issue count mismatch");
    end

    @(posedge clk);
    #1;
    if (!req_in_ready) begin
        $fatal(1, "request_controller should return to ready after MDV scenarios");
    end

    $display("tb_tree_control_stage2_sram_advanced_features_full PASS");
    $finish;
end
