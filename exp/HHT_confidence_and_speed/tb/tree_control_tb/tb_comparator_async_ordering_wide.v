...
module tb_comparator_async_ordering_wide;
...
localparam integer FLUSH_NODE_BIT_C1 =
    (BRANCH_ABC1 * `MAX_VERIFY_NODES_PER_BRANCH) + NODE_C1;
...
task expect_flush_window_closed;
    ...
    begin
        ...
        if (!flush_freeze ||
            ...
            (prefetch_flush_branch_mask != 4'b0101) ||
            tree_in_ready ||
            prefix_ready ||
            frontier_ready ||
...
initial begin
    ...
    issue_branch_and_complete("phase_b_longest_survivor_issue", BRANCH_ABC1D1, NODE_D1);
    ...
    expect_branch_blocked("phase_c_shorter_branch_still_blocked", BRANCH_ABC1, NODE_C1);
    ...
    start_flush_window();
    expect_prefix_state_c1d1("phase_d_flush_keeps_longest_path", 4'b0000);
    expect_flush_window_closed("phase_d_flush_accept_closed");
    expect_branch_blocked("phase_d_longest_survivor_blocked_by_freeze", BRANCH_ABC1D1, NODE_D1);

    @(posedge clk);
    #1;
    hold_flush_window();
    expect_prefix_state_c1d1("phase_d_flush_hold_keeps_longest_path", 4'b0000);
    expect_branch_blocked("phase_d_shorter_branch_still_dead", BRANCH_ABC1, NODE_C1);

    @(posedge clk);
    #1;
    clear_flush_window();

    issue_branch_and_complete("phase_e_longest_survivor_post_flush", BRANCH_ABC1D1, NODE_D1);
    expect_branch_blocked("phase_e_shorter_branch_post_flush", BRANCH_ABC1, NODE_C1);
    expect_branch_blocked("phase_e_incompatible_branch_post_flush", BRANCH_ABC2D2, NODE_D2);
    expect_prefix_state_c1d1("phase_e_final_prefix_stable", 4'b0000);

    $display("tb_comparator_async_ordering_wide PASS");
    $finish;
end
