...
module tb_control_chip_second_flush_reclosure;

localparam integer CFG_W = 32;
localparam [CFG_W-1:0] RUN_043_WAVE1_CFG_DATA = 32'h0430_0001;
localparam [CFG_W-1:0] RUN_043_WAVE2_CFG_DATA = 32'h0430_0002;
...
task expect_wave2_preflush_consume;
    begin
        ...
        for (cycle_i = 0; cycle_i < WAVE2_PREFLUSH_TIMEOUT_CYCLES;
            ...
            if (hbm_req_valid && hbm_req_write) begin
                $fatal(1,
                       "wave2: second-flush proof must not require a second top-level writeback closure");
            end
...
task expect_wave2_postflush_survivor_continuation;
    begin
        ...
        for (cycle_i = 0; cycle_i < WAVE2_POSTFLUSH_TIMEOUT_CYCLES;
            ...
            if (wave2_seen_postflush_read_done && !busy) begin
        ...
        if (busy) begin
            $fatal(1, "wave2: expected busy to clear after second-flush closure");
        end
    end
endtask
...
initial begin
    ...
    launch_bounded_scenario(RUN_043_WAVE1_CFG_DATA);
    ...
    launch_bounded_scenario(RUN_043_WAVE2_CFG_DATA);
    ...
    if (error_flag) begin
        $fatal(1, "control_chip should complete second-flush re-closure without error");
    end

    $display("tb_control_chip_second_flush_reclosure PASS");
    $finish;
end
