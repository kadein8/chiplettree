...
module tb_control_chip_post_second_flush_reentry;

localparam integer CFG_W = 32;
localparam [CFG_W-1:0] RUN_045_WAVE1_CFG_DATA = 32'h0430_0001;
// Wave 2 keeps the already-proven run_044 writeback-closure contract.
localparam [CFG_W-1:0] RUN_045_WAVE2_CFG_DATA = 32'h0440_0002;
// Wave 3 uses its own bounded config so run_045 can prove one more re-entry
// wave without silently widening into run_046 or run_047.
localparam [CFG_W-1:0] RUN_045_WAVE3_CFG_DATA = 32'h0450_0003;
...
localparam integer WAVE3_COMPLETION_TIMEOUT_CYCLES = 160;
...
task expect_wave3_reentry_completion;
    reg seen_completion;
    begin
        seen_completion = 1'b0;
        wave3_seen_prep_done = 1'b0;
        wave3_seen_read_done = 1'b0;

        for (cycle_i = 0; cycle_i < WAVE3_COMPLETION_TIMEOUT_CYCLES;
             cycle_i = cycle_i + 1) begin
            @(posedge clk);
            #1;
            ...
            if (error_flag) begin
                $fatal(1,
                       "wave3: control_chip raised error_flag before post-second-flush re-entry completion");
            end

            if (hbm_req_valid && hbm_req_write) begin
                $fatal(1,
                       "wave3: third-wave re-entry must not emit a third top-level writeback request");
            end
            ...
            if (u_control_chip.u_prediction_unit_bounded_stub.flush_done ||
                ...
                u_control_chip.flush_reclaim_valid) begin
                $fatal(1,
                       "wave3: post-second-flush re-entry must not require a third flush event");
            end
            ...
            if (u_control_chip.u_compute_module_bounded_stub.survivor_read_done) begin
                wave3_seen_read_done = 1'b1;
                seen_completion = 1'b1;
            end

            if (seen_completion && !busy) begin
                disable expect_wave3_reentry_completion;
            end
        end

        if (!wave3_seen_prep_done) begin
            $fatal(1, "wave3: expected prep_done to re-assert after the third launch");
        end

        if (!wave3_seen_read_done) begin
            $fatal(1,
                   "wave3: expected one bounded PE-side completion after the third launch");
        end

        if (busy) begin
            $fatal(1, "wave3: expected busy to clear after the third completion wave");
        end
    end
endtask
...
initial begin
    ...
    launch_bounded_scenario(RUN_045_WAVE1_CFG_DATA);
    ...
    launch_bounded_scenario(RUN_045_WAVE2_CFG_DATA);
    ...
    launch_bounded_scenario(RUN_045_WAVE3_CFG_DATA);
    expect_busy_assert("wave3");
    wave3_seen_busy = 1'b1;
    expect_wave3_reentry_completion();
    ...
    if (!wave3_seen_read_done) begin
        $fatal(1, "wave3: expected one bounded third-wave PE-side completion");
    end

    if (error_flag) begin
        $fatal(1,
               "control_chip should complete post-second-flush re-entry without error");
    end

    $display("tb_control_chip_post_second_flush_reentry PASS");
    $finish;
end
