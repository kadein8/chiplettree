...
module tb_fp16_layer_scheduler;

localparam integer ADDR_W = `SRAM_ADDR_W;
localparam integer HBM_ADDR_W = `HBM_ADDR_W;
...
localparam [ADDR_W-1:0] RESULT_BASE = 23'd2048;
localparam [ADDR_W-1:0] WEIGHT_SRAM_BASE = 23'd8192;
...
logic [ADDR_W-1:0] sram_rd_addr_pending_r;
...
fp16_layer_scheduler #(
    ...
    .HEAD_DIM(HEAD_DIM),
    .N_LAYERS(N_LAYERS),
    .WEIGHT_WINDOW_BEATS(1024),
    .LAYER_WEIGHT_STRIDE(16384)
) u_dut (
    .clk(clk),
    .rst_n(rst_n),
    .issue_valid(issue_valid),
    .issue_ready(issue_ready),
    .issue_token_position(32'd0),
    .issue_input_addr(INPUT_BASE),
    .issue_result_addr(RESULT_BASE),
    .issue_weight_sram_base(WEIGHT_SRAM_BASE),
    .issue_kv_cache_base(KV_CACHE_BASE),
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        sram_rd_addr_pending_r <= {ADDR_W{1'b0}};
        ...
        if (hbm_rd_pending_r) begin
            hbm_resp_valid <= 1'b1;
            hbm_resp_data <= {16{FP_ONE_OVER_128}};
            hbm_rd_pending_r <= 1'b0;
        end
    end
end
    ...
    begin
        for (beat_idx_i = 0; beat_idx_i < HIDDEN_BEATS; beat_idx_i = beat_idx_i + 1) begin
            for (elem_idx_i = 0; elem_idx_i < ELEMS_PER_BEAT; elem_idx_i = elem_idx_i + 1)
                mem[INPUT_BASE + beat_idx_i][(elem_idx_i*DATA_WIDTH) +: DATA_WIDTH] = FP_ONE;
        end
    end
endtask
...
task automatic issue_once;
    begin
        ...
        if (!result_valid)
            $fatal(1, "layer_scheduler timeout");
    end
endtask
...
task automatic check_result_region;
    begin
        ...
        for (beat_idx_i = 0; beat_idx_i < HIDDEN_BEATS; beat_idx_i = beat_idx_i + 1) begin
            for (elem_idx_i = 0; elem_idx_i < ELEMS_PER_BEAT; elem_idx_i = elem_idx_i + 1) begin
                if (is_fp16_nan(mem[RESULT_BASE + beat_idx_i][(elem_idx_i*DATA_WIDTH) +: DATA_WIDTH])) begin
                    $fatal(1, "layer_scheduler result has NaN beat=%0d elem=%0d value=%h",
                           beat_idx_i,
                           elem_idx_i,
        ...
        if (nonzero_count_i == 0)
            $fatal(1, "layer_scheduler result region is all zero");
    end
endtask
...
initial begin
    ...
    if (result_addr !== RESULT_BASE)
        $fatal(1, "layer_scheduler result_addr mismatch actual=%0d expected=%0d",
               result_addr, RESULT_BASE);
    if (current_layer_debug !== (N_LAYERS - 1))
        $fatal(1, "layer_scheduler current_layer_debug mismatch actual=%0d expected=%0d",
               current_layer_debug, N_LAYERS - 1);

    check_result_region();

    $display("tb_fp16_layer_scheduler PASS");
    $finish;
end
