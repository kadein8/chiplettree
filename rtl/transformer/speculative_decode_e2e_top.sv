...
always @(posedge clk) begin
    ...
    // Debug weight parallel mode stall (print first 5 cycles only)
    if (u_layer_ctrl.weight_parallel_mode_r && dbg_cycle_cnt[3:0] == 4'd0)
        $display("[DBG c%0d] WEIGHT_PAR: vec_valid=%h vec_ready=%h mc_valid=%h resp_got=%h rc_state=%0d",
            dbg_cycle_cnt, lc_vec_req_valid, lc_vec_req_ready,
            lc_mc_resp_valid, u_layer_ctrl.weight_resp_got_r, u_req_ctrl.state_r);
