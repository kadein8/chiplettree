...
module tb_fp16_transformer_layer;
...
localparam [ADDR_W-1:0] PRE_NORM_GAMMA_BASE  = 23'd2048;
localparam [ADDR_W-1:0] POST_NORM_GAMMA_BASE = 23'd4096;
localparam [ADDR_W-1:0] WQ_BASE              = 23'd6144;
localparam [ADDR_W-1:0] WK_BASE              = 23'd12288;
localparam [ADDR_W-1:0] WV_BASE              = 23'd18432;
localparam [ADDR_W-1:0] WO_BASE              = 23'd24576;
localparam [ADDR_W-1:0] GATE_W_BASE          = 23'd30720;
localparam [ADDR_W-1:0] UP_W_BASE            = 23'd36864;
localparam [ADDR_W-1:0] DOWN_W_BASE          = 23'd43008;
localparam [ADDR_W-1:0] KV_CACHE_BASE        = 23'd65536;
localparam [ADDR_W-1:0] RESULT_BASE          = 23'd81920;
localparam [ADDR_W-1:0] SCRATCH_BASE         = 23'd90112;
...
fp16_transformer_layer #(
    ...
    .issue_pre_norm_gamma_addr(PRE_NORM_GAMMA_BASE),
    .issue_post_norm_gamma_addr(POST_NORM_GAMMA_BASE),
...
task automatic issue_once;
    begin
        ...
        if (!result_valid)
            $fatal(1, "transformer_layer timeout");
    end
endtask
...
task automatic check_result_region;
    begin
        ...
        for (beat_idx_i = 0; beat_idx_i < HIDDEN_BEATS; beat_idx_i = beat_idx_i + 1) begin
            for (elem_idx_i = 0; elem_idx_i < ELEMS_PER_BEAT; elem_idx_i = elem_idx_i + 1) begin
                if (is_fp16_nan(mem[RESULT_BASE + beat_idx_i][(elem_idx_i*DATA_WIDTH) +: DATA_WIDTH])) begin
                    $fatal(1, "transformer_layer result has NaN beat=%0d elem=%0d value=%h",
                           beat_idx_i,
                           elem_idx_i,
        ...
        if (nonzero_count_i == 0)
            $fatal(1, "transformer_layer result region is all zero");
    end
endtask
...
initial begin
    ...
    fill_region(PRE_NORM_GAMMA_BASE, HIDDEN_BEATS, FP_ONE);
    fill_region(POST_NORM_GAMMA_BASE, HIDDEN_BEATS, FP_ONE);
    fill_region(WQ_BASE, MATVEC_BEATS, FP_ONE_OVER_128);
    fill_region(WK_BASE, MATVEC_BEATS, FP_ONE_OVER_128);
    fill_region(WV_BASE, MATVEC_BEATS, FP_ONE_OVER_128);
    fill_region(WO_BASE, MATVEC_BEATS, FP_ONE_OVER_128);
    ...
    if (result_status !== 2'b00)
        $fatal(1, "transformer_layer result_status mismatch actual=%b", result_status);
    if (result_addr !== RESULT_BASE)
        $fatal(1, "transformer_layer result_addr mismatch actual=%0d expected=%0d",
               result_addr, RESULT_BASE);

    check_result_region();

    $display("tb_fp16_transformer_layer PASS");
    $finish;
end
