...
module tb_control_chip_stage2_single_chiplet_tree_window_multibranch_lifecycle_top_min;
...
localparam [SOURCE_ID_W-1:0] SLOT0_SOURCE_ID = 3'd2;
localparam [`TOKEN_ID_W-1:0] SLOT0_TOKEN_ID = 16'hd101;
localparam [`TOKEN_ID_W-1:0] SLOT0_REF_TOKEN_ID = 16'h8d10;
localparam [`POSITION_ID_W-1:0] SLOT0_REF_POS = 12'h020;
localparam [CONF_W-1:0] SLOT0_CONF = 8'd250;

localparam [SOURCE_ID_W-1:0] SLOT1_SOURCE_ID = 3'd4;
localparam [`TOKEN_ID_W-1:0] SLOT1_TOKEN_ID = 16'hd141;
localparam [`TOKEN_ID_W-1:0] SLOT1_REF_TOKEN_ID = SLOT0_REF_TOKEN_ID;
localparam [`POSITION_ID_W-1:0] SLOT1_REF_POS = SLOT0_REF_POS;
localparam [CONF_W-1:0] SLOT1_CONF = 8'd220;

localparam [SOURCE_ID_W-1:0] SLOT2_SOURCE_ID = 3'd1;
localparam [`TOKEN_ID_W-1:0] SLOT2_TOKEN_ID = 16'hd121;
localparam [`TOKEN_ID_W-1:0] SLOT2_REF_TOKEN_ID = 16'h9d21;
localparam [`POSITION_ID_W-1:0] SLOT2_REF_POS = 12'h03c;
localparam [CONF_W-1:0] SLOT2_CONF = 8'd180;
...
task expect_ready_mask;
    ...
    begin
        ...
        if ((hht_cand_ready !== expected_hht_ready) ||
            ...
            $fatal(1, "18 top ready mask mismatch");
        end
    end
endtask
...
task expect_window_slot;
    ...
    begin
        ...
        if (tree_window_slot_valid[slot_idx] !== expected_valid) begin
            $fatal(1, "18 tree window slot valid mismatch");
        end
        if (expected_valid) begin
            if ((tree_window_source_id[(slot_idx*SOURCE_ID_W) +: SOURCE_ID_W] !=
                ...
                $fatal(1, "18 tree window slot payload mismatch");
            end
        end
    end
endtask
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (tree_window_valid &&
            ...
            (tree_window_slot_valid == 4'b0111) &&
            (tree_window_source_id[0 +: SOURCE_ID_W] == SLOT0_SOURCE_ID) &&
            (tree_window_token_id[0 +: `TOKEN_ID_W] == SLOT0_TOKEN_ID) &&
            (tree_window_referenced_token_id[0 +: `TOKEN_ID_W] ==
             SLOT0_REF_TOKEN_ID) &&
            (tree_window_referenced_position[0 +: `POSITION_ID_W] ==
             SLOT0_REF_POS) &&
            (tree_window_confidence[0 +: CONF_W] == SLOT0_CONF) &&
            ...
            (tree_window_referenced_position[(1*`POSITION_ID_W) +:
                                             `POSITION_ID_W] == SLOT1_REF_POS) &&
            (tree_window_confidence[(1*CONF_W) +: CONF_W] == SLOT1_CONF) &&
            ...
            (tree_window_referenced_position[(2*`POSITION_ID_W) +:
                                             `POSITION_ID_W] == SLOT2_REF_POS) &&
            (tree_window_confidence[(2*CONF_W) +: CONF_W] == SLOT2_CONF)) begin
        ...
        if (recompute_req_valid && recompute_req_ready &&
            ...
            if ((recompute_req_token_id == SLOT0_TOKEN_ID) &&
                (recompute_req_referenced_position == SLOT0_REF_POS)) begin
                recompute_req_count_slot0_r <= recompute_req_count_slot0_r + 1;
            end
            if ((recompute_req_token_id == SLOT1_TOKEN_ID) &&
                (recompute_req_referenced_position == SLOT1_REF_POS)) begin
                recompute_req_count_slot1_r <= recompute_req_count_slot1_r + 1;
            end
            if ((recompute_req_token_id == SLOT2_TOKEN_ID) &&
                (recompute_req_referenced_position == SLOT2_REF_POS)) begin
        ...
        if (kv_lookup_valid && kv_lookup_ready &&
            kv_lookup_hit &&
            (kv_lookup_token_id == SLOT0_REF_TOKEN_ID) &&
            (kv_lookup_position_id == SLOT0_REF_POS) &&
        ...
        if (wb_valid &&
            ...
            if ((wb_token_id == SLOT0_TOKEN_ID) &&
                (wb_data == EXP_PARTIAL_WB_DATA)) begin
                wb_slot0_payload_count_r <= wb_slot0_payload_count_r + 1;
            end
            if ((wb_token_id == SLOT1_TOKEN_ID) &&
                (wb_data == EXP_FULL_WB_DATA)) begin
                wb_slot1_payload_count_r <= wb_slot1_payload_count_r + 1;
            end
            if ((wb_token_id == SLOT2_TOKEN_ID) &&
                (wb_data == EXP_FULL_WB_DATA)) begin
        ...
        if (wb_done) begin
            ...
            if (wb_token_id == SLOT0_TOKEN_ID) begin
                wb_slot0_done_count_r <= wb_slot0_done_count_r + 1;
            end
            if (wb_token_id == SLOT1_TOKEN_ID) begin
                wb_slot1_done_count_r <= wb_slot1_done_count_r + 1;
            end
            if (wb_token_id == SLOT2_TOKEN_ID) begin
...
initial begin
    ...
    if (busy !== 1'b1) begin
        $fatal(1, "18 top did not enter busy");
    end
    ...
    if (!tree_window_seen_r) begin
        $fatal(1, "18 consumed tree window was not observed");
    end
    if ((tree_window_parent_node_id != WINDOW_PARENT_NODE_ID) ||
        (tree_window_slot_valid != 4'b0111)) begin
        $fatal(1, "18 tree window header mismatch");
    end
    expect_window_slot(0, 1'b1, SLOT0_SOURCE_ID, SLOT0_TOKEN_ID,
                       SLOT0_REF_TOKEN_ID, SLOT0_REF_POS, SLOT0_CONF);
    expect_window_slot(1, 1'b1, SLOT1_SOURCE_ID, SLOT1_TOKEN_ID,
                       SLOT1_REF_TOKEN_ID, SLOT1_REF_POS, SLOT1_CONF);
    expect_window_slot(2, 1'b1, SLOT2_SOURCE_ID, SLOT2_TOKEN_ID,
                       SLOT2_REF_TOKEN_ID, SLOT2_REF_POS, SLOT2_CONF);
    ...
    cycle_count = 0;
    while ((recompute_req_count_slot0_r != 1) && (cycle_count < 12)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (recompute_req_count_slot0_r != 1) begin
        $fatal(1, "18 slot0 recompute request was not observed");
    end
    if (recompute_req_count_slot1_r != 0) begin
        $fatal(1, "18 slot1 should not request recompute before reuse");
    end
    if (recompute_req_count_slot2_r != 0) begin
        $fatal(1, "18 slot2 should not request recompute");
    end
    if (!debug_stale_hit) begin
        $fatal(1, "18 slot0 stale hit was not observed");
    end
    if (!debug_recompute_busy) begin
        $fatal(1, "18 recompute control did not enter busy on slot0");
    end
    ...
    if (!recompute_resp_ready) begin
        $fatal(1, "18 recompute response channel was not ready for partial");
    end
    send_partial_kv_response_once();

    cycle_count = 0;
    while (!kv_partial_seen_r && (cycle_count < 20)) begin
        query_kv_tables_once(SLOT0_REF_TOKEN_ID, SLOT0_REF_POS);
        cycle_count = cycle_count + 1;
    end
    if (!kv_partial_seen_r) begin
        $fatal(1, "18 slot0 partial-ready KV state was not observed");
    end
    ...
    if (wb_slot0_payload_count_r != 1) begin
        $fatal(1, "18 slot0 partial writeback payload was not observed");
    end
    ...
    if (wb_slot0_done_count_r != 1) begin
        $fatal(1, "18 slot0 wb_done was not observed");
    end
    if (busy !== 1'b1) begin
        $fatal(1, "18 top should remain busy after slot0 partial writeback");
    end
    if (recompute_done_count_r != 0) begin
        $fatal(1, "18 slot0 partial stage should not finish full recompute");
    end
    ...
    if (!recompute_resp_ready) begin
        $fatal(1, "18 recompute response channel was not ready for full");
    end
    ...
    if (!kv_full_seen_r) begin
        $fatal(1, "18 slot0 full-ready KV state was not observed");
    end
    if (recompute_done_count_r != 1) begin
        $fatal(1, "18 slot0 full recompute completion pulse was not observed");
    end

    cycle_count = 0;
    while ((wb_slot1_payload_count_r != 1) && (cycle_count < 64)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (wb_slot1_payload_count_r != 1) begin
        $fatal(1, "18 slot1 full-ready reuse writeback payload was not observed");
    end

    cycle_count = 0;
    while ((wb_slot1_done_count_r != 1) && (cycle_count < 8)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (wb_slot1_done_count_r != 1) begin
        $fatal(1, "18 slot1 wb_done was not observed");
    end
    if (busy !== 1'b1) begin
        $fatal(1, "18 top should remain busy while slot2 is still pending");
    end
    if (recompute_req_count_slot0_r != 1) begin
        $fatal(1, "18 slot0 recompute count changed unexpectedly");
    end
    if (recompute_req_count_slot1_r != 0) begin
        $fatal(1, "18 slot1 should reuse full-ready KV without recompute");
    end
    if (recompute_req_count_slot2_r != 0) begin
        $fatal(1, "18 slot2 should still have no recompute request");
    end
    ...
    if (wb_slot2_payload_count_r != 1) begin
        $fatal(1, "18 slot2 non-stale writeback payload was not observed");
    end
    ...
    if (wb_slot2_done_count_r != 1) begin
        $fatal(1, "18 slot2 wb_done was not observed");
    end

    cycle_count = 0;
    while ((wb_done_total_count_r != 3) && (cycle_count < 8)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (wb_done_total_count_r != 3) begin
        $fatal(1, "18 combined run should complete exactly three writebacks");
    end
    if (recompute_req_count_slot0_r != 1) begin
        $fatal(1, "18 combined run should keep exactly one slot0 recompute request");
    end
    if (recompute_req_count_slot1_r != 0) begin
        $fatal(1, "18 slot1 incorrectly emitted recompute request");
    end
    if (recompute_req_count_slot2_r != 0) begin
        $fatal(1, "18 slot2 non-stale path incorrectly emitted recompute request");
    end
    ...
    if (busy !== 1'b0) begin
        $fatal(1, "18 top did not return to idle after all window slots completed");
    end

    if (decoder_qkv_count_r < 3) begin
        $fatal(1, "18 decoder qkv stage should be observed three times");
    end
    if (decoder_score_count_r < 3) begin
        $fatal(1, "18 decoder score stage should be observed three times");
    end
    if (decoder_softmax_count_r < 3) begin
        $fatal(1, "18 decoder softmax stage should be observed three times");
    end
    if (decoder_value_count_r < 3) begin
        $fatal(1, "18 decoder value stage should be observed three times");
    end
    if (decoder_ffn_count_r < 3) begin
        $fatal(1, "18 decoder ffn stage should be observed three times");
    end
    if (error_flag !== 1'b0) begin
        $fatal(1, "18 top error flag should stay low");
    end
    if (wb_error !== 1'b0) begin
        $fatal(1, "18 writeback error should stay low");
    end
    if (hbm_req_valid !== 1'b0) begin
        $fatal(1, "18 minimal path should not issue HBM request");
    end
    if (hbm_req_write !== 1'b0) begin
        $fatal(1, "18 minimal path HBM write flag should stay low");
    end
    if (hbm_req_addr != {`HBM_ADDR_W{1'b0}}) begin
        $fatal(1, "18 minimal path HBM address should stay zero");
    end
    if (hbm_req_wdata != {`HBM_DATA_W{1'b0}}) begin
        $fatal(1, "18 minimal path HBM data should stay zero");
    end
    if (hbm_req_id != {`REQ_ID_W{1'b0}}) begin
        $fatal(1, "18 minimal path HBM id should stay zero");
    end

    $display("tb_control_chip_stage2_single_chiplet_tree_window_multibranch_lifecycle_top_min PASS");
    $finish;
end
