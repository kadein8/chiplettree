...
SUMMARY_FILE="${RUN_DIR}/logs/step2_summary.txt"
...
run_vcs_testbench \
    ...
    "exp/sram_ablation_experiment/step2_throughput/tb_exp_step2.v"
rc=$?

{
    echo "run_name=exp_step2_throughput"
    echo "timestamp=$(date '+%Y-%m-%d %H:%M:%S')"
    echo "running=run_exp_step2.sh"
    echo "scope=control_chip_stage2_single_chiplet_ten_round_throughput"
    echo "exit_code_run_exp_step2.sh=${rc}"
    if [[ ${rc} -eq 0 ]]; then
        echo "judge=PASS"
    else
        echo "judge=FAIL"
