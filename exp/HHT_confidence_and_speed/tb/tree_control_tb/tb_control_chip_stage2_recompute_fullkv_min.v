...
module tb_control_chip_stage2_recompute_fullkv_min;
...
localparam [`TOKEN_ID_W-1:0] SEL_TOKEN_ID = 16'h4404;
localparam [`TOKEN_ID_W-1:0] SEL_REF_TOKEN_ID = 16'h8448;
...
localparam [`SRAM_WDATA_W-1:0] EXP_WDATA =
    128'h1234_5678_9abc_def0_0fed_cba9_8765_4321;
...
reg recompute_write_seen_r;
reg recompute_done_seen_r;
reg issue_seen_r;
reg transform_read_seen_r;
reg wb_payload_seen_r;
reg kv_lookup_seen_r;
wire recompute_write_fire_w;
...
task send_full_kv_response_once;
    begin
        ...
        recompute_resp_kv_data = EXP_WDATA;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (recompute_write_fire_w) begin
            recompute_write_seen_r <= 1'b1;
        end
        ...
        if (issue_valid && issue_ready &&
            ...
            if (!(recompute_write_seen_r || recompute_write_fire_w)) begin
                $fatal(1, "issue fired before recompute KV write");
            end
            issue_seen_r <= 1'b1;
        end
...
initial begin
    ...
    if (!recompute_resp_ready) begin
        $fatal(1, "recompute response channel was not ready");
    end

    send_full_kv_response_once();

    cycle_count = 0;
    while ((!recompute_done_seen_r || !recompute_write_seen_r) &&
    ...
    if (!recompute_done_seen_r) begin
        $fatal(1, "recompute completion was not observed");
    end
    if (!recompute_write_seen_r) begin
        $fatal(1, "recompute KV SRAM write was not observed");
    end

    query_kv_tables_once();
    if (!kv_lookup_seen_r) begin
        $fatal(1, "KV table lookup did not return full-ready location");
    end
    ...
    if (!issue_seen_r) begin
        $fatal(1, "tree issue metadata did not match recompute-selected token");
    end
    ...
    $display("tb_control_chip_stage2_recompute_fullkv_min PASS");
    $finish;
end
