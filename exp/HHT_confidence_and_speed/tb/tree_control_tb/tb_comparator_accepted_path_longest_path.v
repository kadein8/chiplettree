...
module tb_comparator_accepted_path_longest_path;
...
localparam [`REQ_ID_W-1:0] TEST_REQ_ID = 4'hC;
...
task expect_first_reduction;
    begin
        ...
        if (!accepted_prefix_valid ||
            ...
            $fatal(1, "first accepted-path reduction mismatch");
        end
    end
endtask

task expect_longest_path_selected;
    begin
        #1;
        if (!accepted_prefix_valid ||
            (accepted_prefix_req_id != TEST_REQ_ID) ||
            ...
            (prune_branch_mask != 4'b0001)) begin
            $fatal(1, "longest-path accepted reduction mismatch");
        end
    end
endtask

task expect_longest_path_stable;
    begin
        #1;
        if (!accepted_prefix_valid ||
            (accepted_prefix_req_id != TEST_REQ_ID) ||
            ...
            (accepted_prefix_node_id[(1*`NODE_ID_W) +: `NODE_ID_W] != NODE_D1) ||
            (live_branch_mask != 4'b0010) ||
            (prune_branch_mask != 4'b0000)) begin
            $fatal(1, "longest-path state should stay stable after deeper acceptance");
        end
    end
endtask
...
initial begin
    ...
    expect_longest_path_selected();

    @(posedge clk);
    #1;
    clear_inputs();
    expect_longest_path_stable();

    $display("tb_comparator_accepted_path_longest_path PASS");
    $finish;
end
