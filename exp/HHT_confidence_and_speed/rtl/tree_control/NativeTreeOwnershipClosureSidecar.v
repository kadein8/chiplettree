...
module NativeTreeOwnershipClosureSidecar #(
    parameter [`REQ_ID_W-1:0] SIDECAR_REQ_ID = 4'hd
) (
    input                             clk,
    input                             rst_n,
    input                             enable,
    ...
    output                            debug_prefix_norm_fire,
    output                            debug_frontier_norm_fire,
...
wire prefix_norm_valid_w;
wire [`REQ_ID_W-1:0] prefix_norm_req_id_w;
wire prefix_norm_node_valid_w;
wire [`NODE_ID_W-1:0] prefix_norm_node_id_w;
wire [`NODE_ID_W-1:0] prefix_norm_parent_node_id_w;
wire [`TOKEN_ID_W-1:0] prefix_norm_token_id_w;
wire [`POSITION_ID_W-1:0] prefix_norm_position_id_w;
wire [`LAYER_ID_W-1:0] prefix_norm_layer_id_w;
wire prefix_norm_is_last_w;
wire prefix_norm_is_shared_w;

wire frontier_norm_valid_w;
wire [`REQ_ID_W-1:0] frontier_norm_req_id_w;
wire [`TREE_LEVEL_ID_W-1:0] frontier_norm_level_id_w;
wire [`TREE_FRONTIER_SLOTS-1:0] frontier_norm_slot_valid_w;
wire [`TREE_FRONTIER_SLOTS*`NODE_ID_W-1:0] frontier_norm_node_id_w;
wire [`TREE_FRONTIER_SLOTS*`NODE_ID_W-1:0] frontier_norm_parent_node_id_w;
wire [`TREE_FRONTIER_SLOTS*`TOKEN_ID_W-1:0] frontier_norm_token_id_w;
wire [`TREE_FRONTIER_SLOTS*`POSITION_ID_W-1:0] frontier_norm_position_id_w;
wire [`TREE_FRONTIER_SLOTS*`KV_GROUP_LEN_W-1:0] frontier_norm_size_subbank_w;
wire [`TREE_FRONTIER_SLOTS-1:0] frontier_norm_slot_shared_w;
...
assign tree_req_valid_w = enable && src_req_valid && !request_active_r;
assign src_req_ready = enable ? (tree_req_ready_w && !request_active_r) : 1'b1;
assign tree_req_fire_w = tree_req_valid_w && tree_req_ready_w;

assign lookup_hit_fire_w = lookup_resp_valid_w && lookup_resp_hit_w &&
                           (lookup_resp_req_id_w == SIDECAR_REQ_ID);

assign pe_req_valid = enable && request_active_r && lookup_hit_seen_r &&
                      !pe_req_sent_r && main_path_quiet;
...
assign pe_resp_ready = enable && request_active_r && pe_req_sent_r &&
                       !pe_resp_seen_r;
assign pe_resp_fire_w = pe_resp_valid && pe_resp_ready &&
                        (pe_resp_req_id == SIDECAR_REQ_ID) &&
                        pe_resp_last;

assign busy = enable && request_active_r;

assign debug_req_fire = tree_req_fire_w;
assign debug_prefix_norm_fire = prefix_norm_valid_w;
assign debug_frontier_norm_fire = frontier_norm_valid_w;
...
tree_analyze u_tree_analyze (
    .clk(clk),
    .rst_n(rst_n),
    .req_valid(tree_req_valid_w),
    .req_ready(tree_req_ready_w),
    .req_id(src_req_id),
    .src_prefix_slot_valid(src_prefix_slot_valid),
    .src_prefix_node_id(src_prefix_node_id),
    .src_prefix_token_id({(`TREE_MAX_PREFIX_NODES*`TOKEN_ID_W){1'b0}}),
    .src_prefix_position_id({(`TREE_MAX_PREFIX_NODES*`POSITION_ID_W){1'b0}}),
    .src_committed_len({`POSITION_ID_W{1'b0}}),
...
agu u_agu (
    ...
    // 该 sidecar 仍沿用旧 prefix/frontier 驱动方式，这里先把新的 bundle 并行入口绑成 0，
    // 避免 AGU 接口升级后出现未连接端口。
    ...
    .prefix_norm_valid(prefix_norm_valid_w),
    .prefix_norm_req_id(prefix_norm_req_id_w),
    .prefix_norm_node_valid(prefix_norm_node_valid_w),
    .prefix_norm_node_id(prefix_norm_node_id_w),
    .prefix_norm_parent_node_id(prefix_norm_parent_node_id_w),
    .prefix_norm_token_id(prefix_norm_token_id_w),
    .prefix_norm_position_id(prefix_norm_position_id_w),
    .prefix_norm_layer_id(prefix_norm_layer_id_w),
    .prefix_norm_is_last(prefix_norm_is_last_w),
    .prefix_norm_is_shared(prefix_norm_is_shared_w),
    .frontier_norm_valid(frontier_norm_valid_w),
    .frontier_norm_req_id(frontier_norm_req_id_w),
    .frontier_norm_level_id(frontier_norm_level_id_w),
    .frontier_norm_slot_valid(frontier_norm_slot_valid_w),
    .frontier_norm_node_id(frontier_norm_node_id_w),
    .frontier_norm_parent_node_id(frontier_norm_parent_node_id_w),
    .frontier_norm_token_id(frontier_norm_token_id_w),
    .frontier_norm_position_id(frontier_norm_position_id_w),
    .frontier_norm_size_subbank(frontier_norm_size_subbank_w),
    .frontier_norm_slot_shared(frontier_norm_slot_shared_w),
...
token_register u_token_register (
    ...
    .lookup_valid(enable && request_active_r && captured_token_valid_r &&
                  !lookup_sent_r),
