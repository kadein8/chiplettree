...
module tb_control_chip_stage2_single_chiplet_hht_writeback_min;
...
localparam [SOURCE_ID_W-1:0] PHASE1_SOURCE_ID = 3'd2;
localparam [SOURCE_ID_W-1:0] PHASE2_SOURCE_ID = {SOURCE_ID_W{1'b0}};
localparam [`NODE_ID_W-1:0] SEL_PARENT_NODE_ID = 4'd5;
localparam [`TOKEN_ID_W-1:0] SEL_TOKEN_ID = 16'hea11;
localparam [`TOKEN_ID_W-1:0] SEL_REF_TOKEN_ID = 16'h8a11;
localparam [`POSITION_ID_W-1:0] SEL_REF_POS = 12'h020;
localparam [CONF_W-1:0] PHASE1_CONF = 8'd208;
localparam [CONF_W-1:0] PHASE2_CONF = 8'd210;
...
reg kv_full_seen_r;
reg [HHT_LRU_W-1:0] lru_after_phase1_write_r;
reg [HHT_LRU_W-1:0] lru_after_phase2_accept_r;
...
task expect_ready_mask;
    ...
    begin
        ...
        if ((hht_cand_ready !== expected_hht_ready) ||
            ...
            $fatal(1, "16 top ready mask mismatch");
        end
    end
endtask

task expect_hht_entry0;
    ...
    begin
        ...
        if (u_control_chip_stage2_single_chiplet
                ...
                .u_hht_state_table.entry_valid_r[0] !== expected_valid) begin
            $fatal(1, "16 HHT entry0 valid mismatch");
        end
        if (expected_valid) begin
            if ((u_control_chip_stage2_single_chiplet
                    .u_integration_prediction_part
                    .u_hht_state_table.entry_parent_node_id_r[0] !=
                 expected_parent_node_id) ||
                (u_control_chip_stage2_single_chiplet
                    .u_integration_prediction_part
                    .u_hht_state_table.entry_token_id_r[0] !=
                 expected_token_id) ||
                (u_control_chip_stage2_single_chiplet
                    .u_integration_prediction_part
                    .u_hht_state_table.entry_referenced_token_id_r[0] !=
                 expected_referenced_token_id) ||
                (u_control_chip_stage2_single_chiplet
                    .u_integration_prediction_part
                    .u_hht_state_table.entry_referenced_position_r[0] !=
                 expected_referenced_position) ||
                (u_control_chip_stage2_single_chiplet
                    .u_integration_prediction_part
                    .u_hht_state_table.entry_confidence_r[0] !=
                 expected_confidence) ||
                (u_control_chip_stage2_single_chiplet
                    .u_integration_prediction_part
                    .u_hht_state_table.entry_hit_count_r[0] !=
                 expected_hit_count)) begin
                $fatal(1, "16 HHT entry0 payload mismatch");
            end
        end
    end
endtask
...
task drive_phase1_candidate_once;
    begin
        hht_cand_valid = 1'b1;
        ...
        draft_confidence[(1*CONF_W) +: CONF_W] = PHASE1_CONF;
...
task drive_phase2_candidate_once;
    begin
        hht_cand_valid = 1'b1;
        hht_parent_node_id = SEL_PARENT_NODE_ID;
        hht_token_id = SEL_TOKEN_ID;
        hht_referenced_token_id = SEL_REF_TOKEN_ID;
        hht_referenced_position = SEL_REF_POS;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (tree_window_valid &&
            (tree_window_parent_node_id == SEL_PARENT_NODE_ID) &&
            tree_window_slot_valid[0] &&
            (tree_window_token_id[0 +: `TOKEN_ID_W] == SEL_TOKEN_ID) &&
            (tree_window_referenced_token_id[0 +: `TOKEN_ID_W] == SEL_REF_TOKEN_ID) &&
            (tree_window_referenced_position[0 +: `POSITION_ID_W] == SEL_REF_POS)) begin
            if ((tree_window_source_id[0 +: SOURCE_ID_W] == PHASE1_SOURCE_ID) &&
                (tree_window_confidence[0 +: CONF_W] == PHASE1_CONF)) begin
                tree_window_phase1_seen_r <= 1'b1;
            end
            if ((tree_window_source_id[0 +: SOURCE_ID_W] == PHASE2_SOURCE_ID) &&
...
initial begin
    clk = 1'b0;
    rst_n = 1'b0;
    lru_after_phase1_write_r = {HHT_LRU_W{1'b0}};
    lru_after_phase2_accept_r = {HHT_LRU_W{1'b0}};
    clear_inputs();
    ...
    expect_hht_entry0(1'b0, {`NODE_ID_W{1'b0}}, {`TOKEN_ID_W{1'b0}},
    ...
    if (busy !== 1'b1) begin
        $fatal(1, "16 phase1 top did not enter busy");
    end

    drive_phase1_candidate_once();
    ...
    if (!tree_window_phase1_seen_r) begin
        $fatal(1, "16 phase1 tree window was not observed");
    end
    ...
    if (recompute_req_count_r != 1) begin
        $fatal(1, "16 phase1 recompute request was not observed");
    end
    if (!debug_stale_hit) begin
        $fatal(1, "16 phase1 stale hit was not observed");
    end
    if (!debug_recompute_busy) begin
        $fatal(1, "16 phase1 recompute control did not enter busy");
    end
    ...
    if (!recompute_resp_ready) begin
        $fatal(1, "16 phase1 recompute response channel was not ready");
    end
    ...
    if (recompute_done_count_r != 1) begin
        $fatal(1, "16 phase1 recompute completion pulse was not observed");
    end
    ...
    if (!kv_full_seen_r) begin
        $fatal(1, "16 phase1 KV full-ready lookup was not observed");
    end

    cycle_count = 0;
    while ((wb_done_count_r != 1) && (cycle_count < 64)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (wb_done_count_r != 1) begin
        $fatal(1, "16 phase1 writeback completion was not observed");
    end
    if (wb_payload_count_r != 1) begin
        $fatal(1, "16 phase1 writeback payload was not observed");
    end
    ...
    expect_hht_entry0(1'b1, SEL_PARENT_NODE_ID, SEL_TOKEN_ID, SEL_REF_TOKEN_ID,
                      SEL_REF_POS, PHASE1_CONF, 8'd0);
    lru_after_phase1_write_r =
        u_control_chip_stage2_single_chiplet
            .u_integration_prediction_part
            .u_hht_state_table.entry_last_use_r[0];
    ...
    if (busy !== 1'b0) begin
        $fatal(1, "16 phase1 top did not return to idle");
    end
    ...
    if (busy !== 1'b1) begin
        $fatal(1, "16 phase2 top did not enter busy");
    end

    drive_phase2_candidate_once();
    expect_hht_entry0(1'b1, SEL_PARENT_NODE_ID, SEL_TOKEN_ID, SEL_REF_TOKEN_ID,
                      SEL_REF_POS, PHASE1_CONF, 8'd1);
    if (!(u_control_chip_stage2_single_chiplet
              .u_integration_prediction_part
              .u_hht_state_table.entry_last_use_r[0] >
          lru_after_phase1_write_r)) begin
        $fatal(1, "16 phase2 accepted HHT hit did not advance LRU");
    end
    lru_after_phase2_accept_r =
        u_control_chip_stage2_single_chiplet
            .u_integration_prediction_part
            .u_hht_state_table.entry_last_use_r[0];
    ...
    if (!tree_window_phase2_seen_r) begin
        $fatal(1, "16 phase2 HHT tree window was not observed");
    end
    consume_tree_window_once();

    cycle_count = 0;
    while ((wb_done_count_r != 2) && (cycle_count < 64)) begin
        @(posedge clk);
        #1;
        cycle_count = cycle_count + 1;
    end
    if (wb_done_count_r != 2) begin
        $fatal(1, "16 phase2 writeback completion was not observed");
    end
    if (wb_payload_count_r != 2) begin
        $fatal(1, "16 phase2 writeback payload was not observed");
    end
    if (recompute_req_count_r != 1) begin
        $fatal(1, "16 phase2 should reuse full-ready KV without recompute request");
    end
    ...
    expect_hht_entry0(1'b1, SEL_PARENT_NODE_ID, SEL_TOKEN_ID, SEL_REF_TOKEN_ID,
                      SEL_REF_POS, PHASE2_CONF, 8'd1);
    if (!(u_control_chip_stage2_single_chiplet
              .u_integration_prediction_part
              .u_hht_state_table.entry_last_use_r[0] >
          lru_after_phase2_accept_r)) begin
        $fatal(1, "16 phase2 writeback refresh did not advance HHT LRU");
    end
    ...
    if (busy !== 1'b0) begin
        $fatal(1, "16 phase2 top did not return to idle");
    end
    if (decoder_qkv_count_r < 2) begin
        $fatal(1, "16 decoder qkv stage should be observed twice");
    end
    if (decoder_score_count_r < 2) begin
        $fatal(1, "16 decoder score stage should be observed twice");
    end
    if (decoder_softmax_count_r < 2) begin
        $fatal(1, "16 decoder softmax stage should be observed twice");
    end
    if (decoder_value_count_r < 2) begin
        $fatal(1, "16 decoder value stage should be observed twice");
    end
    if (decoder_ffn_count_r < 2) begin
        $fatal(1, "16 decoder ffn stage should be observed twice");
    end
    if (error_flag !== 1'b0) begin
        $fatal(1, "16 top error flag should stay low");
    end
    if (wb_error !== 1'b0) begin
        $fatal(1, "16 writeback error should stay low");
    end
    if (hbm_req_valid !== 1'b0) begin
        $fatal(1, "16 minimal path should not issue HBM request");
    end
    if (hbm_req_write !== 1'b0) begin
        $fatal(1, "16 minimal path HBM write flag should stay low");
    end
    if (hbm_req_addr != {`HBM_ADDR_W{1'b0}}) begin
        $fatal(1, "16 minimal path HBM address should stay zero");
    end
    if (hbm_req_wdata != {`HBM_DATA_W{1'b0}}) begin
        $fatal(1, "16 minimal path HBM data should stay zero");
    end
    if (hbm_req_id != {`REQ_ID_W{1'b0}}) begin
        $fatal(1, "16 minimal path HBM id should stay zero");
    end

    $display("tb_control_chip_stage2_single_chiplet_hht_writeback_min PASS");
    $finish;
end
