...
module tb_tree_stage_abc_integration ();
...
task wait_for_alloc_and_token;
    ...
    begin
        ...
        for (wait_i = 0; wait_i < 12; wait_i = wait_i + 1) begin
            ...
            if (token_wr_valid) begin
                if (token_wr_req_id != TEST_REQ_ID ||
                    ...
                    $fatal(1, "Stage ABC token write mismatch");
                end
                if (!alloc_resp_valid ||
                    !alloc_resp_grant ||
                    ...
                    $fatal(1, "Stage ABC bank_state_table response mismatch");
                end
        ...
        if (!seen) begin
            $fatal(1, "Stage ABC integration did not reach alloc/token side effects");
        end
    end
endtask
        ...
        for (wait_i = 0; wait_i < 4; wait_i = wait_i + 1) begin
            ...
            if (lookup_resp_valid) begin
                ...
                if (!lookup_resp_hit || lookup_resp_req_id != TEST_REQ_ID) begin
                    $fatal(1, "Stage ABC token lookup response mismatch");
                end
                disable wait_for_lookup_hit;
            end
        end
        if (!seen) begin
            $fatal(1, "Stage ABC token lookup did not return");
        end
    end
endtask
...
initial begin
    ...
    if (!mem_req_ready[SRAM_LANE]) begin
        $fatal(1, "Stage ABC lookup address should be writable through Stage C");
    end
    ...
    #1;
    if (!mem_req_ready[SRAM_LANE]) begin
        $fatal(1, "Stage ABC lookup address should be readable through Stage C");
    end
    ...
    if (!mem_resp_valid[SRAM_LANE]) begin
        $fatal(1, "Stage ABC Stage C readback did not return");
    end
    ...
    if (mem_resp_id[(SRAM_LANE*`REQ_ID_W) +: `REQ_ID_W] != 4'hb ||
        ...
        $fatal(1, "Stage ABC Stage C readback mismatch");
    end

    $display("tb_tree_stage_abc_integration PASS");
    $finish;
end
