...
module tb_comparator_alloc_status_flush_path;
...
task expect_queue_count;
    input [255:0] scenario_name;
    input integer expect_count;
    begin
        #1;
        if (u_prefetch_queue.entry_count_r != expect_count) begin
            $fatal(1, "%0s: queue count mismatch", scenario_name);
        end
    end
endtask

task expect_queue_slot0_payload;
    input [255:0] scenario_name;
    input [`REQ_ID_W-1:0] req_id;
    input [`BRANCH_ID_W-1:0] branch_id;
    input [`NODE_ID_W-1:0] node_id;
    begin
        #1;
        if (!u_prefetch_queue.entry_valid_r[0] ||
            (u_prefetch_queue.entry_req_id_r[0] != req_id) ||
            (u_prefetch_queue.entry_branch_id_r[0] != branch_id) ||
            (u_prefetch_queue.entry_node_id_r[0] != node_id) ||
            (u_prefetch_queue.entry_size_subbank_r[0] != `KV_GROUP_SIZE_SUBBANK) ||
            u_prefetch_queue.entry_shared_r[0]) begin
            $fatal(1, "%0s: queue slot0 payload mismatch", scenario_name);
        end
    end
endtask

task expect_queue_empty;
    input [255:0] scenario_name;
    begin
        #1;
        if (u_prefetch_queue.entry_count_r != 0 ||
            u_prefetch_queue.entry_valid_r[0] ||
            queue_deq_valid) begin
            $fatal(1, "%0s: queue should be empty", scenario_name);
        end
    end
endtask
...
initial begin
    ...
    hold_queue_deq();
    drive_scalar_req(4'h3, 2'd1, 4'd2);
    wait_for_queue_head("queue_victim_before_flush", 4'h3, 2'd1, 4'd2);
    expect_queue_count("queue_has_one_victim", 1);
    expect_queue_slot0_payload("queue_slot0_victim", 4'h3, 2'd1, 4'd2);
    ...
    @(posedge clk);
    #1;
    expect_queue_empty("queue_victim_removed_by_flush");
    clear_comparator_inputs();
    ...
    $display("tb_comparator_alloc_status_flush_path PASS");
    $finish;
end
