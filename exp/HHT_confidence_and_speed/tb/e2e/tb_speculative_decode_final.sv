...
// tb_speculative_decode_final
//
// Testbench for the final product architecture:
// 4 PE groups parallel verification + multi-token commit.
//
// Expected behavior:
//   - One verification round commits multiple tokens (if predictions correct)
//   - Minimum 1 token per round (Branch 0's generation, always correct)
//   - With toy model and HHT warmup [0,13,6,14,11], expect multi-token commit
...
// DUT
// =========================================================================
speculative_decode_final_top u_dut (
    .clk(clk), .rst_n(rst_n),
    .start(start),
...
initial begin
    ...
    // Direct SRAM preload (single shared SRAM)
    ...
    // Start
    $display("Starting FINAL speculative decode, prompt=%0d, max_gen=%0d",
    ...
    if (dut_done) begin
        $display("\nFinal decode completed in %0d cycles", wait_cycles);
        $display("Generated %0d tokens", token_count);
        if (token_count >= 2)
            $display("tb_speculative_decode_final PASS (multi-token commit!)");
        else if (token_count > 0)
            $display("tb_speculative_decode_final PASS (single-token, predictions may have missed)");
        else
            $display("tb_speculative_decode_final FAIL (no tokens)");
    end else begin
        $display("tb_speculative_decode_final FAIL (timeout after %0d cycles)", wait_cycles);
    end
...
always @(posedge clk) begin
    if (u_dut.tb_done)
        $display("[FINAL_DBG c%0d] tree_builder done: branch_valid=%b", dbg_cnt, u_dut.tb_branch_valid);
    if (u_dut.sched_all_done)
        $display("[FINAL_DBG c%0d] scheduler all_done", dbg_cnt);
    if (u_dut.cmp_result_valid)
        $display("[FINAL_DBG c%0d] comparator result: count=%0d", dbg_cnt, u_dut.cmp_accepted_count);
    if (u_dut.emit_done)
        $display("[FINAL_DBG c%0d] emitter done: %0d tokens", dbg_cnt, u_dut.emit_tokens_emitted);
end
