...
// pe_residual_add_compute — Element-wise FP16 vector addition, compute-only
//
...
module pe_residual_add_compute #(
...
import "DPI-C" function void dpi_residual_add(
    input  bit [NUM_SLOTS*DIM*DATA_W-1:0] vec_a_flat,
    input  bit [NUM_SLOTS*DIM*DATA_W-1:0] vec_b_flat,
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (start) begin
            dpi_residual_add(vec_a, vec_b, active_slots, dpi_out, NUM_SLOTS, DIM);
            vec_out <= dpi_out;
            done <= 1'b1;
        end
    end
