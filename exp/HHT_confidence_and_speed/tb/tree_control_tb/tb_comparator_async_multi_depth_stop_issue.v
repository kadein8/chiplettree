...
module tb_comparator_async_multi_depth_stop_issue;
...
task load_frontier_work;
    begin
        @(posedge clk);
        #1;
        if (!frontier_ready) begin
            $fatal(1, "frontier should be accepted before stop-issue update");
        end
        frontier_valid = 1'b1;
        frontier_req_id = TEST_REQ_ID;
        frontier_level_id = 2'd0;
        frontier_slot_valid = 4'b1111;
        frontier_node_id[(0*`NODE_ID_W) +: `NODE_ID_W] = NODE_C1;
        frontier_parent_node_id[(0*`NODE_ID_W) +: `NODE_ID_W] = NODE_NONE;
        frontier_token_id[(0*`TOKEN_ID_W) +: `TOKEN_ID_W] = 16'h0101;
        frontier_position_id[(0*`POSITION_ID_W) +: `POSITION_ID_W] = 12'h001;
        frontier_node_id[(1*`NODE_ID_W) +: `NODE_ID_W] = NODE_D1;
        frontier_parent_node_id[(1*`NODE_ID_W) +: `NODE_ID_W] = NODE_C1;
        frontier_token_id[(1*`TOKEN_ID_W) +: `TOKEN_ID_W] = 16'h0102;
        frontier_position_id[(1*`POSITION_ID_W) +: `POSITION_ID_W] = 12'h002;
        frontier_node_id[(2*`NODE_ID_W) +: `NODE_ID_W] = NODE_C2;
        frontier_parent_node_id[(2*`NODE_ID_W) +: `NODE_ID_W] = NODE_NONE;
        frontier_token_id[(2*`TOKEN_ID_W) +: `TOKEN_ID_W] = 16'h0201;
        frontier_position_id[(2*`POSITION_ID_W) +: `POSITION_ID_W] = 12'h003;
        frontier_node_id[(3*`NODE_ID_W) +: `NODE_ID_W] = NODE_D2;
        frontier_parent_node_id[(3*`NODE_ID_W) +: `NODE_ID_W] = NODE_C2;
        frontier_token_id[(3*`TOKEN_ID_W) +: `TOKEN_ID_W] = 16'h0202;
        frontier_position_id[(3*`POSITION_ID_W) +: `POSITION_ID_W] = 12'h004;
        @(posedge clk);
        #1;
        frontier_valid = 1'b0;
        frontier_slot_valid = {`TREE_FRONTIER_SLOTS{1'b0}};
...
task expect_reduction_result;
    begin
        ...
        if (!accepted_prefix_valid ||
            ...
            $fatal(1, "comparator reduction result mismatch before stop-issue check");
        end
    end
endtask
...
task expect_dispatch_and_complete;
    ...
    begin
        ...
        for (wait_i = 0; wait_i < 20; wait_i = wait_i + 1) begin
            ...
            if (prefetch_enq_valid) begin
                if ((prefetch_enq_req_id != TEST_REQ_ID) ||
                    (prefetch_enq_branch_id != expect_branch_id) ||
                    (prefetch_enq_node_id != expect_node_id)) begin
                    $fatal(1, "%0s: unexpected dispatch payload", scenario_name);
                end
                seen = 1'b1;
                wait_i = 20;
            end
        ...
        if (!seen) begin
            $display("%0s debug: treeq_count=%0d head_valid=%0b head_req=%0d head_branch=%0d head_node=%0d state=%0d live_mask=%b pending_req=%0d pending_branch=%0d pending_node=%0d",
                     scenario_name,
                     u_agu.treeq_count_r,
                     u_agu.treeq_head_valid_comb,
                     u_agu.treeq_head_req_id_comb,
                     u_agu.treeq_head_branch_id_comb,
                     u_agu.treeq_head_node_id_comb,
                     u_agu.agu_state_r,
                     u_agu.branch_liveness_live_mask_r,
                     u_agu.pending_req_id_r,
                     u_agu.pending_branch_id_r,
                     u_agu.pending_node_id_r);
            $fatal(1, "%0s: expected survivor dispatch missing", scenario_name);
        end
        complete_candidate_and_alloc(TEST_REQ_ID);
    end
endtask

task expect_no_killed_dispatch;
    integer wait_i;
    begin
        for (wait_i = 0; wait_i < 12; wait_i = wait_i + 1) begin
            @(negedge clk);
            #1;
            if (prefetch_enq_valid &&
                ((prefetch_enq_branch_id == BRANCH_ABC2) ||
                 (prefetch_enq_branch_id == BRANCH_ABC2D2))) begin
                $fatal(1, "killed branch should not issue new AGU work");
            end
        end
    end
endtask
...
initial begin
    ...
    expect_reduction_result();

    prefetch_enq_ready = 1'b1;
    expect_dispatch_and_complete("survivor_branch0", BRANCH_ABC1, NODE_C1);
    expect_dispatch_and_complete("survivor_branch1", BRANCH_ABC1D1, NODE_D1);
    expect_no_killed_dispatch();

    $display("tb_comparator_async_multi_depth_stop_issue PASS");
    $finish;
end
