...
module tb_control_chip_third_flush_writeback_closure;

localparam integer CFG_W = 32;
localparam [CFG_W-1:0] RUN_047_WAVE1_CFG_DATA = 32'h0430_0001;
// Wave 2 keeps the already-proven run_044 writeback-closure contract.
localparam [CFG_W-1:0] RUN_047_WAVE2_CFG_DATA = 32'h0440_0002;
// Wave 3 uses its own bounded config so run_047 can demand one bounded
// post-third-flush writeback closure without overloading run_046 semantics.
localparam [CFG_W-1:0] RUN_047_WAVE3_CFG_DATA = 32'h0470_0003;
...
task expect_wave3_postflush_writeback_closure;
    ...
    begin
        ...
        for (cycle_i = 0; cycle_i < WAVE3_WRITEBACK_TIMEOUT_CYCLES;
            ...
            if (error_flag) begin
                $fatal(1,
                       "wave3: control_chip raised error_flag before post-third-flush writeback");
            end

            if (!busy && !seen_writeback) begin
                $fatal(1,
                       "wave3: busy cleared before the post-third-flush writeback closure completed");
            end
        ...
        if (!seen_writeback) begin
            $fatal(1,
                   "wave3: expected one bounded post-third-flush top-level writeback request");
        end

        if (wave3_write_count != 1) begin
            $fatal(1,
                   "wave3: expected exactly one post-third-flush top-level writeback request");
        end

        if (busy) begin
            $fatal(1,
                   "wave3: expected busy to clear after the post-third-flush writeback closure");
        end
    end
endtask
...
initial begin
    ...
    launch_bounded_scenario(RUN_047_WAVE1_CFG_DATA);
    ...
    launch_bounded_scenario(RUN_047_WAVE2_CFG_DATA);
    ...
    launch_bounded_scenario(RUN_047_WAVE3_CFG_DATA);
    ...
    if (!wave3_seen_postflush_writeback) begin
        $fatal(1, "wave3: expected one bounded post-third-flush writeback closure");
    end

    if (error_flag) begin
        $fatal(1,
               "control_chip should complete third-flush writeback closure without error");
    end

    $display("tb_control_chip_third_flush_writeback_closure PASS");
    $finish;
end
