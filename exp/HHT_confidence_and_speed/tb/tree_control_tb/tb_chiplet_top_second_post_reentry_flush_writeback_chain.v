...
module tb_control_chip_second_post_reentry_flush_writeback_chain;

localparam integer CFG_W = 32;
localparam [CFG_W-1:0] RUN_051_WAVE1_CFG_DATA = 32'h0480_0003;
localparam [CFG_W-1:0] RUN_051_WAVE2_CFG_DATA = 32'h04a0_0001;
localparam [CFG_W-1:0] RUN_051_WAVE3_CFG_DATA = 32'h04b0_0001;
...
localparam integer WAVE3_FLUSH_TIMEOUT_CYCLES = 80;
localparam integer WAVE3_POSTFLUSH_TIMEOUT_CYCLES = 160;
localparam integer WAVE3_WRITEBACK_TIMEOUT_CYCLES = 160;
    ...
    begin
        for (cycle_i = 0; cycle_i < QUIET_GAP_CYCLES; cycle_i = cycle_i + 1) begin
            ...
            if (busy || error_flag || hbm_req_valid || hbm_req_write) begin
                $fatal(1, "%0s: expected quiet idle boundary", phase_name);
            end
            if (u_control_chip.u_prediction_unit_bounded_stub.flush_done ||
                u_control_chip.u_compute_module_bounded_stub.survivor_read_done ||
                u_control_chip.token_flush_valid ||
                u_control_chip.flush_reclaim_valid) begin
                $fatal(1, "%0s: expected no cross-wave activity in quiet idle boundary",
                       phase_name);
            end
        end
    end
...
task expect_quiet_gap_after_wave2;
    ...
    begin
        for (cycle_i = 0; cycle_i < QUIET_GAP_CYCLES; cycle_i = cycle_i + 1) begin
            ...
            // `flush_done` in prediction_unit_bounded_stub is sticky until the
            // next scenario_start, so this gap should only forbid genuinely new
            // wave-3 activity, not the completed wave-2 flush marker.
            if (u_control_chip.u_compute_module_bounded_stub.survivor_read_done ||
                u_control_chip.token_flush_valid ||
                u_control_chip.flush_reclaim_valid) begin
                $fatal(1, "%0s: expected no new wave3 activity in quiet idle boundary",
                       phase_name);
            end
        end
    end
...
task expect_wave3_preflush_consume;
    begin
        ...
        for (cycle_i = 0; cycle_i < WAVE3_PREFLUSH_TIMEOUT_CYCLES;
            ...
            if (hbm_req_valid && hbm_req_write) begin
                $fatal(1,
                       "wave3: new writeback must not arrive before the new flush checkpoint");
            end
            ...
            if (u_control_chip.u_prediction_unit_bounded_stub.flush_done ||
                ...
                u_control_chip.flush_reclaim_valid) begin
                $fatal(1,
                       "wave3: new flush arrived before the required preflush consume step");
            end
        ...
        if (!wave3_seen_prep_done) begin
            $fatal(1, "wave3: expected prep_done to assert before the new flush");
        end

        if (!wave3_seen_preflush_read_done) begin
            $fatal(1,
                   "wave3: expected one bounded PE-side completion before the new flush");
        end
    end
endtask

task expect_wave3_flush_reclosure;
    begin
        wave3_seen_flush_done = 1'b0;
        wave3_seen_token_flush = 1'b0;
        wave3_seen_reclaim_handoff = 1'b0;
        wave3_flush_pulse_count = 0;

        for (cycle_i = 0; cycle_i < WAVE3_FLUSH_TIMEOUT_CYCLES;
             cycle_i = cycle_i + 1) begin
            @(posedge clk);
            #1;

            if (error_flag) begin
                $fatal(1, "wave3: control_chip raised error_flag during flush wait");
            end

            if (!busy &&
                !(wave3_seen_flush_done &&
                  wave3_seen_token_flush &&
                  wave3_seen_reclaim_handoff)) begin
                $fatal(1,
                       "wave3: busy cleared before the new flush re-closure was observed");
            end

            if (hbm_req_valid && hbm_req_write) begin
                $fatal(1,
                       "wave3: new writeback must not arrive before the new flush re-closure completes");
            end

            if (u_control_chip.u_prediction_unit_bounded_stub.flush_done &&
                !wave3_seen_flush_done) begin
                wave3_seen_flush_done = 1'b1;
            ...
            if (wave3_seen_flush_done &&
                wave3_seen_token_flush &&
                wave3_seen_reclaim_handoff) begin
                disable expect_wave3_flush_reclosure;
            end
        end

        if (!wave3_seen_flush_done) begin
            $fatal(1, "wave3: expected exactly one new flush marker after preflush consume");
        end

        if (wave3_flush_pulse_count != 1) begin
            $fatal(1, "wave3: expected exactly one new flush marker pulse");
        end

        if (!wave3_seen_token_flush) begin
            $fatal(1, "wave3: expected new metadata-path flush activity");
        end

        if (!wave3_seen_reclaim_handoff) begin
            $fatal(1, "wave3: expected new allocation/status reclaim handoff");
        end
    end
endtask
...
task expect_wave3_postflush_survivor_continuation;
    begin
        ...
        for (cycle_i = 0; cycle_i < WAVE3_POSTFLUSH_TIMEOUT_CYCLES;
            ...
            if (!busy && !wave3_seen_postflush_read_done) begin
                $fatal(1,
                       "wave3: busy cleared before the postflush survivor continuation completed");
            end

            if (hbm_req_valid && hbm_req_write) begin
                $fatal(1,
                       "wave3: new writeback must not arrive before the postflush survivor continuation");
            end
        ...
        if (!wave3_seen_postflush_read_done) begin
            $fatal(1,
                   "wave3: expected one later survivor-only continuation after the new flush");
        end
    end
endtask

task expect_wave3_postflush_writeback_closure;
    reg seen_writeback;
    begin
        seen_writeback = 1'b0;
        wave3_write_count = 0;
        wave3_seen_postflush_writeback = 1'b0;

        for (cycle_i = 0; cycle_i < WAVE3_WRITEBACK_TIMEOUT_CYCLES;
             cycle_i = cycle_i + 1) begin
            @(posedge clk);
            #1;
            ...
            if (error_flag) begin
                $fatal(1,
                       "wave3: control_chip raised error_flag before postflush writeback");
            end

            if (!busy && !seen_writeback) begin
                $fatal(1,
                       "wave3: busy cleared before the postflush writeback closure completed");
            end

            if (hbm_req_valid && hbm_req_write) begin
                if (hbm_req_addr == {`HBM_ADDR_W{1'b0}}) begin
                    $fatal(1, "wave3: expected non-zero bounded writeback address");
                end
                if (hbm_req_wdata == {`HBM_DATA_W{1'b0}}) begin
                    $fatal(1, "wave3: expected non-zero bounded writeback payload");
                end
                if (hbm_req_id == {`REQ_ID_W{1'b0}}) begin
                    $fatal(1, "wave3: expected non-zero bounded writeback req_id");
                end
                wave3_write_count = wave3_write_count + 1;
                wave3_seen_postflush_writeback = 1'b1;
                seen_writeback = 1'b1;
            end

            if (seen_writeback && !busy) begin
                disable expect_wave3_postflush_writeback_closure;
            end
        end

        if (!seen_writeback) begin
            $fatal(1,
                   "wave3: expected one bounded postflush top-level writeback request");
        end

        if (wave3_write_count != 1) begin
            $fatal(1,
                   "wave3: expected exactly one bounded postflush top-level writeback request");
        end

        if (busy) begin
            $fatal(1, "wave3: expected busy to clear after the new writeback closure");
        end
    end
endtask
...
initial begin
    ...
    launch_bounded_scenario(RUN_051_WAVE1_CFG_DATA);
    ...
    expect_quiet_gap_after_wave1("gap_after_wave1");

    launch_bounded_scenario(RUN_051_WAVE2_CFG_DATA);
    ...
    expect_quiet_gap_after_wave2("gap_after_wave2");

    launch_bounded_scenario(RUN_051_WAVE3_CFG_DATA);
    expect_busy_assert("wave3");
    expect_wave3_preflush_consume();
    expect_wave3_flush_reclosure();
    expect_wave3_postflush_survivor_continuation();
    expect_wave3_postflush_writeback_closure();

    if (!wave3_seen_prep_done) begin
        $fatal(1, "wave3: expected one bounded preparation step before the new flush");
    end

    if (!wave3_seen_preflush_read_done) begin
        $fatal(1, "wave3: expected one bounded preflush PE-side completion");
    end

    if (!wave3_seen_flush_done) begin
        $fatal(1, "wave3: expected one bounded new flush marker");
    end
    ...
    if (!wave3_seen_postflush_read_done) begin
        $fatal(1, "wave3: expected one bounded postflush survivor continuation");
    end

    if (!wave3_seen_postflush_writeback) begin
        $fatal(1, "wave3: expected one bounded postflush writeback closure");
    end

    if (error_flag) begin
        $fatal(1,
               "control_chip should complete second post-reentry flush/writeback chain without error");
    end

    $display("tb_control_chip_second_post_reentry_flush_writeback_chain PASS");
    $finish;
end
